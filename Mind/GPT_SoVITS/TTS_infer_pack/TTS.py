...
from Mind.GPT_SoVITS.module.models import SynthesizerTrn, SynthesizerTrnV3, Generator
from Mind.GPT_SoVITS.module.mel_processing import mel_spectrogram_torch, spectrogram_torch
...
class TTS_Config:
    default_configs = {
        "v1": {
            ...
            "is_half": False,
            "version": "v1",
            "t2s_weights_path": "Mind/GPT_SoVITS/pretrained_models/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt",
            "vits_weights_path": "Mind/GPT_SoVITS/pretrained_models/s2G488k.pth",
            "cnhuhbert_base_path": "Mind/GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "Mind/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
        },
        "v2": {
            "device": "cpu",
            "is_half": False,
            "version": "v2",
            "t2s_weights_path": "Mind/GPT_SoVITS/pretrained_models/gsv-v2final-pretrained/s1bert25hz-5kh-longer-epoch=12-step=369668.ckpt",
            "vits_weights_path": "Mind/GPT_SoVITS/pretrained_models/gsv-v2final-pretrained/s2G2333k.pth",
            "cnhuhbert_base_path": "Mind/GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "Mind/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
        },
        "v3": {
            ...
            "is_half": False,
            "version": "v3",
            "t2s_weights_path": "Mind/GPT_SoVITS/pretrained_models/s1v3.ckpt",
            "vits_weights_path": "Mind/GPT_SoVITS/pretrained_models/s2Gv3.pth",
            "cnhuhbert_base_path": "Mind/GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "Mind/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
        },
        "v4": {
            ...
            "version": "v4",
            "t2s_weights_path": "Mind/GPT_SoVITS/pretrained_models/s1v3.ckpt",
            "vits_weights_path": "Mind/GPT_SoVITS/pretrained_models/gsv-v4-pretrained/s2Gv4.pth",
            "cnhuhbert_base_path": "Mind/GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "Mind/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
        },
...
class TTS:
    ...
    def init_vocoder(self, version: str):
        if version == "v3":
            ...
            state_dict_g = torch.load(
                "%s/Mind/GPT_SoVITS/pretrained_models/gsv-v4-pretrained/vocoder.pth" % (now_dir,), map_location="cpu")
    ...
    def init_sr_model(self):
        if self.sr_model is not None:
            return
        try:
            self.sr_model: AP_BWE = AP_BWE(
                self.configs.device, DictToAttrRecursive)
    ...
    def run(self, inputs: dict):
        ...
        if no_prompt_text and self.configs.use_vocoder:
            raise NO_PROMPT_ERROR(
                "prompt_text cannot be empty when using SoVITS_V3")

        if ref_audio_path in [None, ""] and (
            (self.prompt_cache["prompt_semantic"] is None) or (
                self.prompt_cache["refer_spec"] in [None, []])
        ):
            raise ValueError(
        ...
        try:
            ...
            t_45 = 0.0
            audio = []
            output_sr = self.configs.sampling_rate if not self.configs.use_vocoder else self.vocoder_configs[
                "sr"]
            for item in data:
                ...
                if no_prompt_text:
                    ...
                    prompt = (
                        self.prompt_cache["prompt_semantic"].expand(
                ...
                if not self.configs.use_vocoder:
                    if speed_factor == 1.0:
                        ...
                        upsample_rate = math.prod(
                            self.vits_model.upsample_rates)
                        audio_frag_idx = [
                        ...
                        all_pred_semantic = (
                            torch.cat(pred_semantic_list).unsqueeze(
                        ...
                        for i, idx in enumerate(tqdm(idx_list)):
                            phones = batch_phones[i].unsqueeze(
                                0).to(self.configs.device)
                            _pred_semantic = (
                                pred_semantic_list[i][-idx:].unsqueeze(
                                    0).unsqueeze(0)
                            ...
                            batch_audio_fragment.append(
                                audio_fragment)  # 试试重建不带上prompt部分
                else:
                    if parallel_infer:
                        print(f"{i18n('并行合成中')}...")
    ...
    def using_vocoder_synthesis(
        ...
        prompt_semantic_tokens = self.prompt_cache["prompt_semantic"].unsqueeze(
            0).unsqueeze(0).to(self.configs.device)
        prompt_phones = torch.LongTensor(
            self.prompt_cache["phones"]).unsqueeze(0).to(self.configs.device)
        refer_audio_spec = self.prompt_cache["refer_spec"][0].to(
        ...
        if ref_sr != tgt_sr:
            ref_audio = resample(
                ref_audio, ref_sr, tgt_sr, self.configs.device)

        mel2 = mel_fn(
            ref_audio) if self.configs.version == "v3" else mel_fn_v4(ref_audio)
        mel2 = norm_spec(mel2)
    ...
    def using_vocoder_synthesis_batched_infer(
        ...
        pred_spec = pred_spec.permute(
            1, 0, 2).contiguous().view(dd, -1).unsqueeze(0)
        ...
        audio = self.sola_algorithm(
            audio_fragments, overlapped_len * upsample_rate)
        audio = audio[overlapped_len *
                      upsample_rate: -padding_len * upsample_rate]
    ...
    def sola_algorithm(
        ...
        for i in range(len(audio_fragments) - 1):
            ...
            corr = F.conv1d(w1.view(1, 1, -1), w2.view(1, 1, -1),
                            padding=w2.shape[-1] // 2).view(-1)[:-1]
            idx = corr.argmax()
