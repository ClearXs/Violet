...
class OpenAIClient(LLMClientBase):
    def _prepare_client_kwargs(self) -> dict:
        ...
        if custom_api_key:
            ...
            api_key = override_key or model_settings.openai_api_key or os.environ.get(
                "OPENAI_API_KEY")
    ...
    def build_request_data(
        ...
        use_developer_message = llm_config.model.startswith(
            "o1") or llm_config.model.startswith("o3")  # o-series models
    ...
    def handle_llm_error(self, e: Exception) -> Exception:
        ...
        if isinstance(e, openai.RateLimitError):
            logger.warning(
                f"[OpenAI] Rate limited (429). Consider backoff. Error: {e}")
        ...
        if isinstance(e, openai.AuthenticationError):
            ...
            logger.error(f"[OpenAI] Authentication error (401): {str(e)}")
        ...
        if isinstance(e, openai.PermissionDeniedError):
            # More severe log level
            logger.error(f"[OpenAI] Permission denied (403): {str(e)}")
        ...
        if isinstance(e, openai.APIStatusError):
            logger.warning(
                f"[OpenAI] API status error ({e.status_code}): {str(e)}")
