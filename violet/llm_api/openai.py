...
from violet.constants import OPENAI_CONTEXT_WINDOW_ERROR_SUBSTRING
...
from violet.schemas.openai.chat_completion_request import FunctionCall as ToolFunctionChoiceFunctionCall
from violet.schemas.openai.chat_completion_request import Tool, ToolFunctionChoice, cast_message_to_subtype
from violet.schemas.openai.chat_completion_response import (
...
def openai_get_model_list(
    ...
    try:
        ...
        try:
            ...
            pass
        printd(
            f"Got RequestException, exception={req_err}, response={response}")
        raise req_err
    ...
    if use_tool_naming:
        if function_call is None:
            ...
            tool_choice = ToolFunctionChoice(
                type="function", function=ToolFunctionChoiceFunctionCall(name=function_call))
        else:
            tool_choice = function_call
        data = ChatCompletionRequest(
            model=model,
            messages=openai_message_list,
            tools=[Tool(type="function", function=f)
                   for f in functions] if functions else None,
...
def openai_chat_completions_process_stream(
    ...
    To "stream" the response in Violet, we want to call a streaming-compatible interface function
    ...
    chat_history = [m.model_dump(exclude_none=True)
                    for m in chat_completion_request.messages]
    # print(chat_history)
    ...
    if chat_completion_request.tools is not None:
        ...
        prompt_tokens += num_tokens_from_functions(
            functions=[t.function.model_dump()
                       for t in chat_completion_request.tools],
            model=chat_completion_request.model,
        )
    elif chat_completion_request.functions is not None:
        assert chat_completion_request.tools is None
        prompt_tokens += num_tokens_from_functions(
            functions=[f.model_dump()
                       for f in chat_completion_request.functions],
            model=chat_completion_request.model,
        )
    ...
    chat_completion_response = ChatCompletionResponse(
        ...
        choices=[],
        # NOTE: doesn't matter since both will do get_utc_time()
    ...
    try:
        for chunk_idx, chat_completion_chunk in enumerate(
            ...
            assert len(chat_completion_chunk.choices) == len(
                chat_completion_response.choices), chat_completion_chunk
            for chunk_choice in chat_completion_chunk.choices:
                ...
                if message_delta.tool_calls is not None:
                    ...
                    if accum_message.tool_calls is None:
                        accum_message.tool_calls = [
                            ToolCall(id=TEMP_STREAM_TOOL_CALL_ID,
                ...
                if message_delta.function_call is not None:
                    raise NotImplementedError(
                        f"Old function_call style not support with stream=True")
    ...
    assert all([c.finish_reason !=
               TEMP_STREAM_FINISH_REASON for c in chat_completion_response.choices])
    assert all(
        [
            all([tc.id != TEMP_STREAM_TOOL_CALL_ID for tc in c.message.tool_calls]
                ) if c.message.tool_calls else True
        ...
        with connect_sse(client, method="POST", url=url, json=data, headers=headers) as event_source:
            ...
            if not event_source.response.is_success:
                ...
                try:
                    ...
                    print(
                        f"Failed to parse SSE message, throwing SSE HTTP error up the stack")
            ...
            try:
                for sse in event_source.iter_sse():
                    ...
                    if sse.data == OPENAI_SSE_DONE:
                        ...
                        chunk_object = ChatCompletionChunkResponse(
                            **chunk_data)
...
def openai_chat_completions_request_stream(
    ...
    if "tools" in data:
        for tool in data["tools"]:
            # tool["strict"] = True
            try:
                tool["function"] = convert_to_structured_output(
                    tool["function"])
