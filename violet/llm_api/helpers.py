...
def _convert_to_structured_output_helper(property: dict) -> dict:
    ...
    if param_type == "object":
        ...
        if "items" not in property:
            raise ValueError(
                f"Property {property} of type array is missing items")
...
def add_inner_thoughts_to_functions(
    ...
    for function_object in functions:
        ...
        if inner_thoughts_required:
            required_params = new_function_object["parameters"].get(
                "required", [])
...
def unpack_all_inner_thoughts_from_kwargs(
    ...
    if len(response.choices) == 0:
        raise ValueError(
            f"Unpacking inner thoughts from empty response not supported")

    new_choices = []
    for choice in response.choices:
        new_choices.append(unpack_inner_thoughts_from_kwargs(
            choice, inner_thoughts_key))
...
def calculate_summarizer_cutoff(in_context_messages: List[Message], token_counts: List[int], logger: "logging.Logger") -> int:
    ...
    in_context_messages_openai = [m.to_openai_dict()
    ...
    if summarizer_settings.evict_all_messages:
        ...
        logger.info(
            f"Evicting {cutoff}/{len(in_context_messages)} messages...")
        return cutoff + 1
...
def get_token_counts_for_messages(in_context_messages: List[Message]) -> List[int]:
    in_context_messages_openai = [m.to_openai_dict()
                                  for m in in_context_messages]
    token_counts = [count_tokens(str(msg))
                    for msg in in_context_messages_openai]
    return token_counts
    ...
    # Based on python requests + OpenAI REST API (/v1)
        ...
        if exception.response is not None and "application/json" in exception.response.headers.get("Content-Type", ""):
            try:
                ...
                printd(
                    f"HTTPError occurred ({exception}), but no JSON error message.")
