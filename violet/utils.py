...
from violet.schemas.openai.chat_completion_request import Tool, ToolCall
...
def enforce_types(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        hints = {k: v for k, v in get_type_hints(
            func).items() if k != "return"}
...
def verify_first_message_correctness(
    ...
    if (hasattr(response_message, "function_call") and response_message.function_call is not None) and (
        hasattr(response_message,
                "tool_calls") and response_message.tool_calls is not None
    ):
        printd(
        ...
        printd(
            f"First message didn't include function call: {response_message}")
        return False
...
class OpenAIBackcompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "openai.openai_object":
            from violet.openai_backcompat.openai_object import OpenAIObject
...
def parse_formatted_time(formatted_time):
    # parse times returned by violet.utils.get_formatted_time()
...
def validate_function_response(function_response_string: any, return_char_limit: int, strict: bool = False, truncate: bool = True) -> str:
    """Check to make sure that a function used by Violet returned a valid response. Truncates to return_char_limit if necessary.
...
def list_agent_config_files(sort="last_modified"):
    ...
    agent_dir = os.path.join(VIOLET_DIR, "agents")
    ...
    files = [file for file in files if os.path.isdir(
        os.path.join(agent_dir, file))]

    if sort is not None:
        if sort == "last_modified":
            # Sort the directories by last modified (most recent first)
            files.sort(key=lambda x: os.path.getmtime(
                os.path.join(agent_dir, x)), reverse=True)
        else:
...
def list_human_files():
    ...
    defaults_dir = os.path.join(violet.__path__[0], "humans", "examples")
    user_dir = os.path.join(VIOLET_DIR, "humans")

    violet_defaults = os.listdir(defaults_dir)
    violet_defaults = [os.path.join(defaults_dir, f)
                       for f in violet_defaults if f.endswith(".txt")]
    ...
    if os.path.exists(user_dir):
        ...
        user_added = []
    return violet_defaults + user_added


def list_persona_files():
    """List all personas files"""
    defaults_dir = os.path.join(violet.__path__[0], "personas", "examples")
    user_dir = os.path.join(VIOLET_DIR, "personas")
...
def get_persona_text(name: str, enforce_limit=True):
    for file_path in list_persona_files():
        ...
        if f"{name}.txt" == file or name == file:
            persona_text = open(
                file_path, "r", encoding="utf-8").read().strip()
...
def get_schema_diff(schema_a, schema_b):
    ...
    difference = list(difflib.ndiff(f_schema_json.splitlines(
        keepends=True), linked_function_json.splitlines(keepends=True)))

    # Filter out lines that don't represent changes
    difference = [line for line in difference if line.startswith(
        "+ ") or line.startswith("- ")]
...
def sanitize_filename(filename: str) -> str:
    ...
    max_base_length = MAX_FILENAME_LENGTH - \
        len(ext) - 33  # 32 for UUID + 1 for `_`
...
def get_friendly_error_msg(function_name: str, exception_name: str, exception_message: str):
    from violet.constants import MAX_ERROR_MESSAGE_CHAR_LIMIT
...
def check_args(run_action, function_args):
    ...
    missing_args = [arg for arg in required_args[run_action]
...
def num_tokens_from_tool_calls(tool_calls: Union[List[dict], List[ToolCall]], model: str = "gpt-4"):
    ...
    for tool_call in tool_calls:
        ...
        function_tokens += 2 + \
            len(encoding.encode(tool_call_function_arguments))
...
def log_telemetry(logger: Logger, event: str, **kwargs):
    ...
    if settings.verbose_telemetry_logging:
        timestamp = datetime.now(timezone.utc).strftime(
            "%Y-%m-%d %H:%M:%S,%f UTC")  # More readable timestamp
        extra_data = " | ".join(
...
def generate_short_id(prefix="id", length=4):
    ...
    random_part = random.choice(
        string.ascii_uppercase) + ''.join(random.choices(chars, k=length-1))
