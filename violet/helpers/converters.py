...
from violet.schemas.agent import AgentStepState
from violet.schemas.embedding_config import EmbeddingConfig
from violet.schemas.enums import ProviderType, ToolRuleType
...
def serialize_tool_returns(tool_returns: Optional[List[Union[ToolReturn, dict]]]) -> List[Dict]:
    ...
    for tool_return in tool_returns:
        if isinstance(tool_return, ToolReturn):
            ...
            # Already a dictionary, leave it as-is
            serialized_tool_returns.append(tool_return)
        else:
            raise TypeError(
                f"Unexpected tool return type: {type(tool_return)}")
...
def serialize_message_content(message_content: Optional[List[Union[MessageContent, dict]]]) -> List[Dict]:
    ...
    for content in message_content:
        if isinstance(content, MessageContent):
            ...
            # Already a dictionary, leave it as-is
            serialized_message_content.append(content)
        else:
            raise TypeError(
                f"Unexpected message content type: {type(content)}")
