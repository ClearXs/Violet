...
from violet.schemas.tool_rule import BaseToolRule, ChildToolRule, ConditionalToolRule, InitToolRule, TerminalToolRule
...
class ToolRulesSolver(BaseModel):
    ...
    def get_allowed_tool_names(self, error_on_empty: bool = False, last_function_response: Optional[str] = None) -> List[str]:
        ...
        if self.last_tool_name is None:
            ...
            current_rule = next(
                (rule for rule in self.tool_rules if rule.tool_name == self.last_tool_name), None)

            if current_rule is None:
                if error_on_empty:
                    raise ValueError(
                        f"No tool rule found for {self.last_tool_name}")
                return []
