...
def extract_and_wrap_message_content(
    ...
    if role == MessageRole.user and wrap_user_message:
        message_content = system.package_user_message(
            user_message=message_content)
    elif role == MessageRole.system and wrap_system_message:
        message_content = system.package_system_message(
...
def prepare_input_message_create(
    ...
    **kwargs
    ...
    if isinstance(message.content, list):
        content = [extract_and_wrap_message_content(
            role=message.role, content=c, **kwargs) for c in message.content]
    else:
        content = [extract_and_wrap_message_content(
