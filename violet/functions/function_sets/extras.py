...
def message_chatgpt(self, message: str):
    ...
    message_sequence = [
        Message(user_id=dummy_user_id, agent_id=dummy_agent_id,
                role="system", text=MESSAGE_CHATGPT_FUNCTION_SYSTEM_MESSAGE),
        Message(user_id=dummy_user_id, agent_id=dummy_agent_id,
...
def read_from_text_file(self, filename: str, line_start: int, num_lines: Optional[int] = 1):
    ...
    if line_start < 1 or num_lines < 1:
        raise ValueError(
            "Both line_start and num_lines must be positive integers.")
