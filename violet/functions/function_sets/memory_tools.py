...
from violet.schemas.knowledge_vault import KnowledgeVaultItemBase
from violet.schemas.episodic_memory import EpisodicEventForLLM
from violet.schemas.resource_memory import ResourceMemoryItemBase
from violet.schemas.procedural_memory import ProceduralMemoryItemBase
from violet.schemas.semantic_memory import SemanticMemoryItemBase


# type: ignore
def core_memory_append(self: "Agent", agent_state: "AgentState", label: str, content: str) -> Optional[str]:
    """
...
# type: ignore
def core_memory_rewrite(self: "Agent", agent_state: "AgentState", label: str, content: str) -> Optional[str]:
    """
...
# type: ignore
def core_memory_replace(self: "Agent", agent_state: "AgentState", label: str, old_line_numbers: str, new_content: str) -> Optional[str]:
    """
    ...
    # Parse line numbers
    # Extract the number part after "Line "
    line_part = old_line_numbers.split(" ")[-1]
    ...
    if new_content != "":
        ...
        current_value = current_value[:start_idx] + \
            new_lines + current_value[end_idx + 1:]
    else:
        # Delete the lines
...
def check_episodic_memory(self: "Agent", event_ids: List[str], timezone_str: str) -> List[EpisodicEventForLLM]:
    ...
    formatted_results = [{'event_id': x.id, 'timestamp': x.occurred_at, 'event_type': x.event_type, 'actor': x.actor,
                          'summary': x.summary, 'details': x.details, 'tree_path': x.tree_path} for x in episodic_memory]
...
def check_semantic_memory(self: "Agent", semantic_item_ids: List[str], timezone_str: str) -> List[SemanticMemoryItemBase]:
    ...
    formatted_results = [{'semantic_item_id': x.id, 'name': x.name, 'summary': x.summary,
                          'details': x.details, 'source': x.source, 'tree_path': x.tree_path} for x in semantic_memory]
...
def semantic_memory_update(self: "Agent", old_semantic_item_ids: List[str], new_items: List[SemanticMemoryItemBase]):
    ...
    message_to_return = "Semantic memory with the following ids have been deleted: " + \
...
def trigger_memory_update_with_instruction(self: "Agent", user_message: object, instruction: str, memory_type: str) -> Optional[str]:
    ...
    client.send_message(agent_id=matching_agent.id,
                        ...
                        retrieved_memories=user_message.get(
                            'retrieved_memories', None)
                        )
    response += '[System Message] Agent ' + matching_agent.name + \
        ...
        for memory_type in memory_types:
            if memory_type in memory_type_to_agent_type:
                valid_agent_types.append(
                    memory_type_to_agent_type[memory_type])
            else:
                raise ValueError(
                    f"Memory type '{memory_type}' is not supported. Please choose from 'core', 'episodic', 'resource', 'procedural', 'knowledge_vault', 'semantic'.")
        ...
        with ThreadPoolExecutor(max_workers=len(valid_agent_types)) as pool:
            ...
            for agent_type in valid_agent_types:
                ...
                if not matching_agents:
                    raise ValueError(
                        f"No agent found with type '{agent_type}'")
                futures.append(
