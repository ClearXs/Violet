...
from violet.errors import VioletToolCreateError
from violet.functions.schema_generator import generate_schema
...
def derive_openai_json_schema(source_code: str, name: Optional[str] = None) -> dict:
    ...
    try:
        ...
        functions = [f for f in env if callable(
            env[f]) and not f.startswith("__")]
        if not functions:
            raise VioletToolCreateError(
                "No callable functions found in source code")
        ...
        try:
            ...
            raise VioletToolCreateError(
                f"Type error in schema generation: {str(e)}")
        except ValueError as e:
            raise VioletToolCreateError(
                f"Value error in schema generation: {str(e)}")
        ...
        traceback.print_exc()
        raise VioletToolCreateError(
            f"Schema generation failed: {str(e)}") from e
...
def _get_module_source(module: ModuleType) -> str:
    ...
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        ...
        if module_name.startswith('violet.functions.function_sets.'):
            file_name = module_name.split('.')[-1] + '.py'
            bundled_path = os.path.join(
                sys._MEIPASS, 'violet', 'functions', 'function_sets', file_name)
    ...
    for attr_name in dir(module):
        ...
        if inspect.isfunction(attr) and attr.__module__ == module.__name__:
            if attr_name in function_dict:
                raise ValueError(
                    f"Found a duplicate of function name '{attr_name}'")
