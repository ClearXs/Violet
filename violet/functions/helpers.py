...
from violet.constants import COMPOSIO_ENTITY_ENV_VAR_KEY, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG
from violet.schemas.enums import MessageRole
from violet.schemas.violet_message import AssistantMessage, ReasoningMessage, ToolCallMessage
...
def generate_langchain_tool_wrapper(
    ...
    extra_module_imports = generate_import_code(
        additional_imports_module_attr_map)
...
def assert_all_classes_are_imported(tool: Union["LangChainBaseTool"], additional_imports_module_attr_map: dict[str, str]) -> None:
    ...
    if additional_imports_module_attr_map:
        current_class_imports.update(
            set(additional_imports_module_attr_map.values()))
...
def generate_imported_tool_instantiation_call_str(obj: Any) -> Optional[str]:
    if isinstance(obj, (int, float, str, bool, type(None))):
        ...
        for arg, value in fields.items():
            python_string = generate_imported_tool_instantiation_call_str(
                value)
            if python_string:
        ...
        list_items = [
            generate_imported_tool_instantiation_call_str(v) for v in obj]
...
def parse_violet_response_for_assistant_message(
    violet_response: VioletResponse,
    assistant_message_tool_name: str = DEFAULT_MESSAGE_TOOL,
    assistant_message_tool_kwarg: str = DEFAULT_MESSAGE_TOOL_KWARG,
) -> Optional[str]:
    reasoning_message = ""
    for m in violet_response.messages:
...
async def async_send_message_with_retries(
    ...
    Args:
        server: The Violet server instance (from get_violet_server()).
    ...
    for attempt in range(1, max_retries + 1):
        try:
            messages = [MessageCreate(
                role=MessageRole.user, text=message_text, name=sender_agent.agent_state.name)]
            # Wrap in a timeout
            ...
            assistant_message = parse_violet_response_for_assistant_message(
                response,
                assistant_message_tool_name=DEFAULT_MESSAGE_TOOL,
                assistant_message_tool_kwarg=DEFAULT_MESSAGE_TOOL_KWARG,
            )
