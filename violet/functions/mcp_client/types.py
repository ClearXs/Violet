...
TEMPLATED_VARIABLE_REGEX = (
    # Allows for optional whitespace around the variable name and default value
...
class MCPToolHealth(BaseModel):
    ...
    reasons: List[str] = Field(
        default_factory=list, description="List of reasons for the health status")
...
class MCPTool(Tool):
    ...
    health: Optional[MCPToolHealth] = Field(
        None, description="Schema health status for OpenAI strict mode")
    ...
    def get_tool_variable(self, value: str, environment_variables: Dict[str, str]) -> Optional[str]:
        ...
        def replace_template(match):
            ...
            env_value = environment_variables.get(
                variable_name) if environment_variables else None
        ...
        if re.search(TEMPLATED_VARIABLE_REGEX, result):
            logger.warning(
                f"Unable to resolve templated variable in value: {value}")
            return value
    ...
    def resolve_custom_headers(
        ...
        for key, value in custom_headers.items():
            ...
            if self.is_templated_tool_variable(value):
                resolved_headers[key] = self.get_tool_variable(
...
class SSEServerConfig(BaseServerConfig):
    ...
    auth_header: Optional[str] = Field(
        None, description="The name of the authentication header (e.g., 'Authorization')")
    auth_token: Optional[str] = Field(
        None, description="The authentication token or API key value")
    custom_headers: Optional[dict[str, str]] = Field(
        None, description="Custom HTTP headers to include with SSE requests")
    ...
    def resolve_environment_variables(self, environment_variables: Optional[Dict[str, str]] = None) -> None:
        if self.auth_token and super().is_templated_tool_variable(self.auth_token):
            self.auth_token = super().get_tool_variable(
                self.auth_token, environment_variables)

        self.custom_headers = super().resolve_custom_headers(
            self.custom_headers, environment_variables)
...
class StreamableHTTPServerConfig(BaseServerConfig):
    ...
    auth_token: Optional[str] = Field(
        None, description="The authentication token or API key value")
    custom_headers: Optional[dict[str, str]] = Field(
        None, description="Custom HTTP headers to include with streamable HTTP requests")
