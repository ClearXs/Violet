...
from violet.functions.mcp_client.exceptions import MCPTimeoutError
from violet.functions.mcp_client.types import BaseServerConfig, MCPTool
...
class BaseMCPClient:
    ...
    def connect_to_server(self):
        ...
        success = self._initialize_connection(
            self.server_config, timeout=tool_settings.mcp_connect_to_server_timeout)

        if success:
            try:
                self.loop.run_until_complete(
                    asyncio.wait_for(self.session.initialize(
                    ), timeout=tool_settings.mcp_connect_to_server_timeout)
                )
                self.initialized = True
            except asyncio.TimeoutError:
                raise MCPTimeoutError(
                    "initializing session", self.server_config.server_name, tool_settings.mcp_connect_to_server_timeout)
        else:
            raise RuntimeError(
    ...
    def _initialize_connection(self, server_config: BaseServerConfig, timeout: float) -> bool:
        raise NotImplementedError(
            "Subclasses must implement _initialize_connection")

    def list_tools(self) -> List[MCPTool]:
        self._check_initialized()
        try:
            response = self.loop.run_until_complete(
                asyncio.wait_for(self.session.list_tools(),
                                 timeout=tool_settings.mcp_list_tools_timeout)
            )
            ...
            raise MCPTimeoutError(
                "listing tools", self.server_config.server_name, tool_settings.mcp_list_tools_timeout)

    def execute_tool(self, tool_name: str, tool_args: dict) -> Tuple[str, bool]:
        self._check_initialized()
        try:
            result = self.loop.run_until_complete(
                asyncio.wait_for(self.session.call_tool(
                    tool_name, tool_args), timeout=tool_settings.mcp_execute_tool_timeout)
            )
            ...
            raise MCPTimeoutError(
                f"executing tool '{tool_name}'", self.server_config.server_name, tool_settings.mcp_execute_tool_timeout)
