...
from violet.functions.mcp_client.base_client import BaseAsyncMCPClient, BaseMCPClient
from violet.functions.mcp_client.types import SSEServerConfig
...
class SSEMCPClient(BaseMCPClient):
    def _initialize_connection(self, server_config: SSEServerConfig, timeout: float) -> bool:
        try:
            ...
            sse_transport = self.loop.run_until_complete(
                asyncio.wait_for(sse_cm.__aenter__(), timeout=timeout))
            self.stdio, self.write = sse_transport
            self.cleanup_funcs.append(lambda: self.loop.run_until_complete(
                sse_cm.__aexit__(None, None, None)))
            ...
            logger.error(
                f"Timed out while establishing SSE connection (timeout={timeout}s).")
            return False
        except Exception:
            logger.exception(
                "Exception occurred while initializing SSE client session.")
            return False
    ...
    async def _initialize_connection(self, server_config: SSEServerConfig, timeout: float) -> bool:
        try:
            ...
            self.cleanup_funcs.append(
                lambda: sse_cm.__aexit__(None, None, None))

            session_cm = ClientSession(self.stdio, self.write)
            self.session = await session_cm.__aenter__()
            self.cleanup_funcs.append(
                lambda: session_cm.__aexit__(None, None, None))
            return True
        except Exception:
            logger.exception(
                "Exception occurred while initializing SSE client session.")
