...
from violet.functions.mcp_client.base_client import BaseMCPClient
from violet.functions.mcp_client.types import StdioServerConfig
...
class StdioMCPClient(BaseMCPClient):
    def _initialize_connection(self, server_config: StdioServerConfig, timeout: float) -> bool:
        try:
            ...
            logger.exception(
                "Exception occurred while initializing stdio client session.")
            return False
...
async def forked_stdio_client(server: StdioServerParameters):
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to spawn process: {server.command} {server.args}") from exc
