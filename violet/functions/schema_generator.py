...
def type_to_json_schema_type(py_type) -> dict:
    ...
    if is_optional(py_type):
        ...
        assert optional_length(
            py_type) == 1, f"Optional type must have exactly one type argument, but got {py_type}"
...
def pydantic_model_to_json_schema(model: Type[BaseModel]) -> dict:
    ...
    def resolve_ref(ref: str, schema: dict) -> dict:
        ...
        if model_name not in schema.get("$defs", {}):
            raise ValueError(
                f"Reference {model_name} not found in schema definitions")
    ...
    def clean_schema(schema_part: dict, full_schema: dict) -> dict:
        ...
        if schema_part["type"] == "object":
            if "properties" not in schema_part:
                raise ValueError(
                    f"Object schema lacks 'properties' key: {schema_part}")
...
def generate_schema(function, name: Optional[str] = None, description: Optional[str] = None) -> dict:
    ...
    for param in sig.parameters.values():
        ...
        if param.name in ["self", "agent_state", 'user_message', 'timezone_str']:
            continue
        ...
        if (
            (inspect.isclass(param.annotation) or inspect.isclass(
            ...
            schema["parameters"]["properties"][param.name] = pydantic_model_to_json_schema(
                param.annotation)
        ...
        #                   however in that
            ...
            if not param_doc:
                ...
                if param.annotation != inspect.Parameter.empty:
                    param_generated_schema = type_to_json_schema_type(
                        param.annotation)
                else:
...
def generate_schema_from_args_schema_v2(
    ...
    for field_name, field in args_schema.model_fields.items():
        ...
        properties[field_name] = type_to_json_schema_type(
