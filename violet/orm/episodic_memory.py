...
from violet.orm.mixins import OrganizationMixin

from violet.schemas.episodic_memory import EpisodicEvent as PydanticEpisodicEvent

from violet.orm.custom_columns import CommonVector, EmbeddingConfigColumn
from violet.constants import MAX_EMBEDDING_DIM
from violet.settings import settings

if TYPE_CHECKING:
    from violet.orm.organization import Organization
...
class EpisodicEvent(SqlalchemyBase, OrganizationMixin):
    ...
    last_modify: Mapped[dict] = mapped_column(
        JSON,
        nullable=False,
        default=lambda: {"timestamp": datetime.now(
    ...
    if settings.violet_pg_uri_no_default:
        ...
        details_embedding = mapped_column(
            Vector(MAX_EMBEDDING_DIM), nullable=True)
    ...
    __table_args__ = tuple(
        filter(None, [
            ...
            ) if settings.violet_pg_uri_no_default else None,
            ...
            Index('ix_episodic_memory_summary_sqlite',
                  'summary') if not settings.violet_pg_uri_no_default else None,
            Index('ix_episodic_memory_details_sqlite',
                  'details') if not settings.violet_pg_uri_no_default else None,
        ])
    )
