...
from violet.orm.base import Base, CommonSqlalchemyMetaMixins
from violet.orm.errors import DatabaseTimeoutError, ForeignKeyConstraintViolationError, NoResultFound, UniqueConstraintViolationError
...
class SqlalchemyBase(CommonSqlalchemyMetaMixins, Base):
        ...
        **kwargs,
        ...
        with db_session as session:
            ...
            if cursor:
                ...
                if not cursor_obj:
                    raise NoResultFound(
                        f"No {cls.__name__} found with id {cursor}")
            ...
            if actor:
                query = cls.apply_access_predicate(
            ...
            if tags and hasattr(cls, "tags"):
                ...
                if match_all_tags:
                    ...
                    query = (
                        query.join(cls.tags).filter(cls.tags.property.mapper.class_.tag.in_(
                            tags)).group_by(cls.id)  # Deduplicate results
                    )
            ...
            if cursor_obj:
                if ascending:
                    query = query.where(cls.created_at >= cursor_obj.created_at).where(
                        or_(cls.created_at > cursor_obj.created_at,
            ...
            if query_text:
                if hasattr(cls, "text"):
                    query = query.filter(func.lower(
                        cls.text).contains(func.lower(query_text)))
                elif hasattr(cls, "name"):
                    # Special case for Agent model - search across name
                    query = query.filter(func.lower(
                        cls.name).contains(func.lower(query_text)))
    ...
    def read(
        ...
        **kwargs,
        ...
        """
        logger.debug(
            f"Reading {cls.__name__} with ID: {identifier} with actor={actor}")
        ...
        if actor:
            query = cls.apply_access_predicate(
                query, actor, access, access_type)
            query_conditions.append(
                f"access level in {access} for actor='{actor}'")
        ...
        conditions_str = ", ".join(
            query_conditions) if query_conditions else "no specific conditions"
    ...
    def size(
        ...
        **kwargs,
        ...
        """
        logger.debug(
            f"Calculating size for {cls.__name__} with filters {kwargs}")
        ...
        with db_session as session:
            ...
            try:
                ...
                logger.exception(
                    f"Failed to calculate size for {cls.__name__}")
                raise e
    ...
    def apply_access_predicate(
        ...
        if access_type == AccessType.ORGANIZATION:
            ...
            if not org_id:
                raise ValueError(
                    f"object {actor} has no organization accessor")
