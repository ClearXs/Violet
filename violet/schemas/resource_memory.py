...
class ResourceMemoryItemBase(VioletBase):
    """
...
class ResourceMemoryItem(ResourceMemoryItemBase):
    ...
    last_modify: Dict[str, Any] = Field(
        default_factory=lambda: {
            "timestamp": get_utc_time().isoformat(), "operation": "created"},
    ...
    metadata_: Dict[str, Any] = Field(
        default_factory=dict, description="Arbitrary additional metadata (tags, creation date, etc.)")
...
class ResourceMemoryItemUpdate(VioletBase):
    ...
    title: Optional[str] = Field(
        None, description="Short name/title of the resource")
    ...
    content: Optional[str] = Field(
        None, description="Full or partial text content")
    tree_path: Optional[List[str]] = Field(
        None,
        description="Hierarchical categorization path as an array of strings (e.g., ['documents', 'work', 'projects'])"
    )
    organization_id: Optional[str] = Field(
        None, description="The organization ID")
