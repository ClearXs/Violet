...
from violet.schemas.enums import ToolRuleType
from violet.schemas.violet_base import VioletBase


class BaseToolRule(VioletBase):
...
class ConditionalToolRule(BaseToolRule):
    ...
    default_child: Optional[str] = Field(
        None, description="The default child tool to be called. If None, any tool can be called.")
    child_output_mapping: Dict[Any, str] = Field(
        ..., description="The output case to check for mapping")
    require_output_mapping: bool = Field(
        default=False, description="Whether to throw an error when output doesn't match any case")
...
ToolRule = Annotated[
    Union[ChildToolRule, InitToolRule, TerminalToolRule, ConditionalToolRule,
          ContinueToolRule, MaxCountPerStepToolRule, ParentToolRule],
