...
from violet.schemas.violet_message_content import (
    VioletAssistantMessageContentUnion,
    VioletUserMessageContentUnion,
    get_violet_assistant_message_content_union_str_json_schema,
    get_violet_user_message_content_union_str_json_schema,
)

# ---------------------------
# Violet API Messaging Schemas
# ---------------------------
...
class VioletMessage(BaseModel):
    """
    Base class for simplified Violet message response type. This is intended to be used for developers
...
class SystemMessage(VioletMessage):
    """
...
class UserMessage(VioletMessage):
    """
    ...
    Args:
        ...
        date (datetime): The date the message was created in ISO format
        name (Optional[str]): The name of the sender of the message
        content (Union[str, List[VioletUserMessageContentUnion]]): The message content sent by the user (can be a string or an array of multi-modal content parts)
    """

    message_type: Literal[MessageType.user_message] = Field(
        MessageType.user_message, description="The type of the message.")
    content: Union[str, List[VioletUserMessageContentUnion]] = Field(
        ...,
        description="The message content sent by the user (can be a string or an array of multi-modal content parts)",
        json_schema_extra=get_violet_user_message_content_union_str_json_schema(),
    )


class ReasoningMessage(VioletMessage):
    """
    ...
    source: Literal["reasoner_model",
                    "non_reasoner_model"] = "non_reasoner_model"
    reasoning: str
    signature: Optional[str] = None


class HiddenReasoningMessage(VioletMessage):
    """
...
class ToolCallMessage(VioletMessage):
    """
    ...
    def validate_tool_call(cls, v):
        ...
        if isinstance(v, dict):
            if "name" in v and "arguments" in v and "tool_call_id" in v:
                ...
                raise ValueError(
                    "tool_call must contain either 'name' or 'arguments'")
        return v


class ToolReturnMessage(VioletMessage):
    """
    A message representing the return value of a tool call (generated by Violet executing the requested tool).
...
class AssistantMessage(VioletMessage):
    """
    ...
    Args:
        ...
        date (datetime): The date the message was created in ISO format
        name (Optional[str]): The name of the sender of the message
        content (Union[str, List[VioletAssistantMessageContentUnion]]): The message content sent by the agent (can be a string or an array of content parts)
    """
    ...
    content: Union[str, List[VioletAssistantMessageContentUnion]] = Field(
        ...,
        description="The message content sent by the agent (can be a string or an array of content parts)",
        json_schema_extra=get_violet_assistant_message_content_union_str_json_schema(),
    )


# NOTE: use Pydantic's discriminated unions feature: https://docs.pydantic.dev/latest/concepts/unions/#discriminated-unions
VioletMessageUnion = Annotated[
    Union[SystemMessage, UserMessage, ReasoningMessage, HiddenReasoningMessage,
          ToolCallMessage, ToolReturnMessage, AssistantMessage],
    Field(discriminator="message_type"),
]


def create_violet_message_union_schema():
    return {
        "oneOf": [
...
VioletMessageUpdateUnion = Annotated[
    Union[UpdateSystemMessage, UpdateUserMessage,
          UpdateReasoningMessage, UpdateAssistantMessage],
    Field(discriminator="message_type"),
]
...
class LegacyFunctionCallMessage(VioletMessage):
    function_call: str


class LegacyFunctionReturn(VioletMessage):
    """
    A message representing the return value of a function call (generated by Violet executing the requested function).
...
class LegacyInternalMonologue(VioletMessage):
    """
...
LegacyVioletMessage = Union[LegacyInternalMonologue,
                            AssistantMessage, LegacyFunctionCallMessage, LegacyFunctionReturn]
