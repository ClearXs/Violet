...
class VioletResponse(BaseModel):
    """
    Response object from an agent interaction, consisting of the new messages generated by the agent and usage statistics.
    The type of the returned messages can be either `Message` or `VioletMessage`, depending on what was specified in the request.

    Attributes:
        messages (List[Union[Message, VioletMessage]]): The messages returned by the agent.
        usage (VioletUsageStatistics): The usage statistics
    """

    messages: List[VioletMessageUnion] = Field(
        ...,
        description="The messages returned by the agent.",
        json_schema_extra={
            "items": {
                "$ref": "#/components/schemas/VioletMessageUnion",
            }
        },
    )
    usage: VioletUsageStatistics = Field(
        ...,
        ...
        return json_dumps(
            {
                ...
                # Assume `Message` and `VioletMessage` have a `dict()` method
                # Assume `VioletUsageStatistics` has a `dict()` method
...
# The streaming response is either [DONE], [DONE_STEP], [DONE], an error, or a VioletMessage
VioletStreamingResponse = Union[VioletMessage,
                                MessageStreamStatus, VioletUsageStatistics]
