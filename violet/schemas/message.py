...
from violet.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, TOOL_CALL_ID_MAX_LEN
from violet.helpers.datetime_helpers import get_utc_time, is_utc_datetime
from violet.helpers.json_helpers import json_dumps
from violet.constants import INNER_THOUGHTS_KWARG
from violet.schemas.enums import MessageRole
from violet.schemas.violet_base import OrmMetadataBase
from violet.schemas.violet_message import (
    AssistantMessage,
...
class MessageCreate(BaseModel):
    ...
    content: Union[str, List[VioletMessageContentUnion]] = Field(
        ...,
        description="The content of the message.",
        json_schema_extra=get_violet_message_content_union_str_json_schema(),
    )
    name: Optional[str] = Field(
        None, description="The name of the participant.")
    otid: Optional[str] = Field(
        None, description="The offline threading id associated with this message")
    sender_id: Optional[str] = Field(
        None, description="The id of the sender of the message, can be an identity id or agent id")
...
class MessageUpdate(BaseModel):
    ...
    content: Optional[Union[str, List[VioletMessageContentUnion]]] = Field(
        None,
        description="The content of the message.",
        json_schema_extra=get_violet_message_content_union_str_json_schema(),
    )
    ...
    tool_call_id: Optional[str] = Field(
        None, description="The id of the tool call.")
...
class Message(BaseMessage):
    """
    Violet's internal representation of a message. Includes methods to convert to/from LLM provider formats.
    ...
    content: Optional[List[VioletMessageContentUnion]] = Field(
    ...
    created_at: datetime = Field(
        default_factory=get_utc_time, description="The timestamp when the object was created.")
    ...
    def to_json(self):
        ...
        if json_message["tool_calls"] is not None:
            json_message["tool_calls"] = [
                vars(tc) for tc in json_message["tool_calls"]]
    ...
    @staticmethod
    def to_violet_messages_from_list(
        ...
        if use_assistant_message:
            ...
            messages = [
                msg for msg in messages if msg.id not in message_ids_to_remove]

        # Convert messages to VioletMessages
        return [
            msg
            for m in messages
    ...
    def to_violet_message(
        ...
        """Convert message object (in DB format) to the style used by the original Violet API"""
        messages = []
        ...
        if self.role == MessageRole.assistant:
            ...
            if self.content:
                ...
                if len(self.content) == 1 and isinstance(self.content[0], TextContent):
                    otid = Message.generate_otid_from_id(
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to decode function return: {text_content}")
    ...
    def dict_to_message(
        ...
        content = [TextContent(text=openai_message_dict["content"])
                   ] if openai_message_dict["content"] else []
    ...
    def to_openai_dict_search_results(self, max_tool_id_length: int = TOOL_CALL_ID_MAX_LEN) -> dict:
        ...
        search_result_json = {"timestamp": self.created_at, "message": {
            "content": result_json["content"], "role": result_json["role"]}}
    ...
    def to_google_ai_dict(self, put_inner_thoughts_in_kwargs: bool = True) -> dict:
        ...
        if self.role == "system":
            ...
            if self.name is None:
                warnings.warn(
                    f"Couldn't find function name on tool call, defaulting to tool ID instead.")
    ...
    def to_cohere_dict(
        ...
        if self.role == "system":
            ...
            """
            raise UserWarning(
                f"role 'system' messages should go in 'preamble' field for Cohere API")

        elif self.role == "user":
            assert all([v is not None for v in [
                       text_content, self.role]]), vars(self)
            cohere_message = [
                {
                    "role": "USER",
            ...
            if text_content and self.tool_calls:
                ...
                raise ValueError(
                    "Message does not have content nor tool_calls")
...
class ToolReturn(BaseModel):
    status: Literal["success",
                    "error"] = Field(..., description="The status of the tool call")
