...
class CloudFileContent(MessageContent):
    ...
    cloud_file_uri: str = Field(...,
                                description="The URI of the file in the database")


VioletUserMessageContentUnion = Annotated[
    Union[TextContent, ImageContent, FileContent, CloudFileContent],
    Field(discriminator="type"),
]


def create_violet_user_message_content_union_schema():
    return {
        "oneOf": [
...
def get_violet_user_message_content_union_str_json_schema():
    return {
        "anyOf": [
            {
                "type": "array",
                "items": {
                    "$ref": "#/components/schemas/VioletUserMessageContentUnion",
                },
            },
            {"type": "string"},
        ],
...
VioletAssistantMessageContentUnion = Annotated[
    Union[TextContent],
    Field(discriminator="type"),
]


def create_violet_assistant_message_content_union_schema():
    return {
        "oneOf": [
            {"$ref": "#/components/schemas/TextContent"},
        ],
...
def get_violet_assistant_message_content_union_str_json_schema():
    return {
        "anyOf": [
            {
                "type": "array",
                "items": {
                    "$ref": "#/components/schemas/VioletAssistantMessageContentUnion",
                },
            },
            {"type": "string"},
        ],
...
VioletMessageContentUnion = Annotated[
    Union[TextContent, ImageContent, FileContent, CloudFileContent, ToolCallContent,
          ToolReturnContent, ReasoningContent, RedactedReasoningContent, OmittedReasoningContent],
    Field(discriminator="type"),
]


def get_violet_message_content_union_str_json_schema():
    return {
        "anyOf": [
            {
                "type": "array",
