...
from violet.functions.functions import derive_openai_json_schema, get_json_schema_from_module
from violet.functions.helpers import generate_langchain_tool_wrapper
from violet.functions.schema_generator import generate_schema_from_args_schema_v2
from violet.orm.enums import ToolType
from violet.schemas.violet_base import VioletBase


class BaseTool(VioletBase):
    __id_prefix__ = "tool"
...
class Tool(BaseTool):
    ...
    tool_type: ToolType = Field(
        ToolType.CUSTOM, description="The type of the tool.")
    description: Optional[str] = Field(
        None, description="The description of the tool.")
    source_type: Optional[str] = Field(
        None, description="The type of the source code.")
    organization_id: Optional[str] = Field(
        None, description="The unique identifier of the organization associated with the tool.")
    ...
    return_char_limit: int = Field(
        FUNCTION_RETURN_CHAR_LIMIT, description="The maximum number of characters in the response.")
    ...
    def populate_missing_fields(self):
        ...
        if self.tool_type == ToolType.CUSTOM:
            ...
            # If it's violet core tool, we generate the json_schema on the fly here
            ...
            # If it's violet multi-agent tool, we also generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(
                module_name=MIRIX_MULTI_AGENT_TOOL_MODULE_NAME, function_name=self.name)
...
class ToolCreate(VioletBase):
    name: Optional[str] = Field(
        None, description="The name of the function (auto-generated from source_code if not provided).")
    description: Optional[str] = Field(
        None, description="The description of the tool.")
    tags: List[str] = Field([], description="Metadata tags.")
    ...
    source_type: str = Field(
        "python", description="The source type of the function.")
    json_schema: Optional[Dict] = Field(
    ...
    def from_langchain(
        ...
        Returns:
            Tool: A Violet Tool initialized with attributes derived from the provided LangChain BaseTool object.
        """
...
class ToolRunFromSource(VioletBase):
    source_code: str = Field(...,
                             description="The source code of the function.")
    ...
    name: Optional[str] = Field(
        None, description="The name of the tool to run.")
    source_type: Optional[str] = Field(
        None, description="The type of the source code.")
