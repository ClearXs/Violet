...
from violet.constants import DEFAULT_EMBEDDING_CHUNK_SIZE
from violet.schemas.block import CreateBlock
from violet.schemas.embedding_config import EmbeddingConfig
from violet.schemas.environment_variables import AgentEnvironmentVariable
...
class AgentState(OrmMetadataBase, validate_assignment=True):
    ...
    message_ids: Optional[List[str]] = Field(
        default=None, description="The ids of the messages in the agent's in-context memory.")
    ...
    organization_id: Optional[str] = Field(
        None, description="The unique identifier of the organization associated with the agent.")
...
class CreateAgent(BaseModel, validate_assignment=True):  #
    ...
    tools: Optional[List[str]] = Field(
        None, description="The tools used by the agent.")
    ...
    include_base_tools: bool = Field(
        True, description="If true, attaches the Violet core tools (e.g. archival_memory and core_memory related functions)."
    )
    include_multi_agent_tools: bool = Field(
        False, description="If true, attaches the Violet multi-agent tools (e.g. sending a message to another agent)."
    )
    description: Optional[str] = Field(
        None, description="The description of the agent.")
    metadata_: Optional[Dict] = Field(
    ...
    embedding_chunk_size: Optional[int] = Field(
        DEFAULT_EMBEDDING_CHUNK_SIZE, description="The embedding chunk size used by the agent.")
    from_template: Optional[str] = Field(
        None, description="The template id used to configure the agent")
    template: bool = Field(
        False, description="Whether the agent is a template")
