...
from violet.server.router.scene import router as SceneRouter
...
class MessageRequest(BaseModel):
    ...
    # Source names corresponding to image_uris
...
async def send_message_endpoint(request: MessageRequest,
    ...
    try:
        ...
        if response == "ERROR":
            raise HTTPException(
                status_code=500, detail="Agent returned an error")
                ...
                try:
                    ...
                    if response is None:
                        ...
                        if request.memorizing:
                            ...
                            print(
                                "[DEBUG] Agent returned empty response unexpectedly")
                            result_queue.put(
                                {"type": "error", "error": "Agent returned empty response"})
                    else:
                        print(
...
async def set_model(request: SetModelRequest,
    ...
    try:
        ...
        custom_models_dir = Path.home() / ".violet" / "custom_models"
        custom_config = None
        ...
        if custom_models_dir.exists():
            ...
            for config_file in custom_models_dir.glob("*.yaml"):
                try:
                    ...
                    print(
                        f"Error reading custom model config {config_file}: {e}")
                    continue

        # Set the model with custom config if found, otherwise use standard method
        if custom_config:
            result = agent.set_model(
...
async def add_custom_model(request: AddCustomModelRequest,
    ...
    try:
        ...
        safe_model_name = "".join(
            c for c in request.model_name if c.isalnum() or c in ('-', '_', '.')).rstrip()
...
async def get_current_timezone(agent: AgentWrapper = Depends(get_agent)):
    ...
    try:
        current_timezone = agent.client.server.user_manager.get_user_by_id(
            agent.client.user.id).timezone
...
async def set_screenshot_setting(request: ScreenshotSettingRequest,
    ...
    try:
        agent.set_include_recent_screenshots(
            request.include_recent_screenshots)
        return ScreenshotSettingResponse(
            success=True,
                ...
                if isinstance(steps, str):
                    try:
                        ...
                        if isinstance(steps, list) and steps and isinstance(steps[0], dict):
                            steps = [step.get('instruction', str(step))
                                     for step in steps]
...
async def clear_conversation_history(agent: AgentWrapper = Depends(get_agent)):
    ...
    try:
        if agent is None:
            raise HTTPException(
                status_code=400, detail="Agent not initialized")
