...
if is_g2pw:
    ...
    from violet.voice.text.g2pw import G2PWPinyin, correct_pronunciation
    from violet.config import VioletConfig

    config = VioletConfig.get_config()

    model_dir = os.path.join(config.model_storage_path,
                             "GPT_SoVITS", "text", "G2PWModel")
    bert_path = os.path.join(config.model_storage_path,
                             "GPT_SoVITS", "chinese-roberta-wwm-ext-large")

    parent_directory = os.path.dirname(current_file_path)
    g2pw = G2PWPinyin(
        model_dir=model_dir,
        model_source=os.environ.get("bert_path", bert_path),
        v_to_u=False,
            ...
            for word, pos in seg_cut:
                ...
                for pinyin in word_pinyins:
                    if pinyin[0].isalpha():
                        ...
                        sub_finals.append(to_finals_tone3(
                            pinyin, neutral_tone_with_five=True))
                    else:
