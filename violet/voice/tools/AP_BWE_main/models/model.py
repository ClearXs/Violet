...
class APNet_BWE_Model(torch.nn.Module):
    def __init__(self, h):
        ...
        self.linear_post_pha_r = nn.Linear(
            h.ConvNeXt_channels, h.n_fft // 2 + 1)
        self.linear_post_pha_i = nn.Linear(
            h.ConvNeXt_channels, h.n_fft // 2 + 1)
    ...
    def forward(self, mag_nb, pha_nb):
        ...
        com_wb = torch.stack((torch.exp(
            mag_wb) * torch.cos(pha_wb), torch.exp(mag_wb) * torch.sin(pha_wb)), dim=-1)
...
class DiscriminatorAR(nn.Module):
    def __init__(
        ...
        self.convs = nn.ModuleList(
            [
                weight_norm(nn.Conv2d(in_channels, channels, kernel_size=(
                    7, 5), stride=(2, 2), padding=(3, 2))),
                weight_norm(nn.Conv2d(channels, channels, kernel_size=(
                    5, 3), stride=(2, 1), padding=(2, 1))),
                weight_norm(nn.Conv2d(channels, channels, kernel_size=(
                    5, 3), stride=(2, 2), padding=(2, 1))),
...
def stft_mag(audio, n_fft=2048, hop_length=512):
    ...
    stft_spec = torch.stft(audio, n_fft, hop_length,
...
def cal_snr(pred, target):
    snr = (20 * torch.log10(torch.norm(target, dim=-1) /
