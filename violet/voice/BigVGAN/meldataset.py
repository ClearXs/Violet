...
# NOTE: 32768.0 -1 to prevent int16 overflow (results in popping sound in corner cases)
...
def mel_spectrogram(
    ...
    if torch.min(y) < -1.0:
        print(
            f"[WARNING] Min value of input waveform signal is {torch.min(y)}")
    if torch.max(y) > 1.0:
        print(
            f"[WARNING] Max value of input waveform signal is {torch.max(y)}")
...
class MelDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.is_seen:
            ...
            self.name = "-".join(pathlib.Path(
                self.audio_files[0]).parts[:2]).strip("/")
        ...
        for i in tqdm(range(len(self.audio_files))):
            assert os.path.exists(
                self.audio_files[i]), f"{self.audio_files[i]} not found"
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, str, torch.Tensor]:
        try:
            ...
            audio, source_sampling_rate = librosa.load(
            ...
            # Shape sanity checks
            assert (
                audio.shape[1] == mel.shape[2] *
                self.hop_size and audio.shape[1] == mel_loss.shape[2] *
                self.hop_size
            ), (
        ...
        # If it encounters error during loading the data, skip this sample and load random other sample to the batch
            ...
            if self.fine_tuning:
                # Terminate training if it is fine-tuning. The dataset should have been prepared properly.
                raise e
            else:
                print(
