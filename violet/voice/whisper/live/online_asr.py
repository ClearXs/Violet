...
from violet.voice.whisper.live.timed_objects import ASRToken, Sentence, Transcript
...
class HypothesisBuffer:
    ...
    def insert(self, new_tokens: List[ASRToken], offset: float):
        ...
        self.new = [token for token in new_tokens if token.start >
                    self.last_committed_time - 0.1]
            ...
            if abs(first_token.start - self.last_committed_time) < 1:
                if self.committed_in_buffer:
                    ...
                    for i in range(1, max_ngram + 1):
                        ...
                        new_ngram = " ".join(
                            token.text for token in self.new[:i])
...
class OnlineASRProcessor:
    ...
    def __init__(
        ...
        if self.buffer_trimming_way not in ["sentence", "segment"]:
            raise ValueError(
                "buffer_trimming must be either 'sentence' or 'segment'")
    ...
    def insert_silence(self, silence_duration, offset):
        ...
        if True:  # silence_duration < 3: #we want the last audio to be treated to not have a gap. could also be handled in the future in ends_with_silence.
            gap_silence = np.zeros(
                int(16000 * silence_duration), dtype=np.int16)
    ...
    def prompt(self) -> Tuple[str, str]:
        ...
        context_text = self.asr.sep.join(
            token.text for token in non_prompt_tokens)
    ...
    def process_iter(self) -> Tuple[List[ASRToken], float]:
        ...
        if not committed_tokens and buffer_duration > self.buffer_trimming_sec:
            time_since_last_output = self.get_audio_buffer_end_time() - \
                self.time_of_last_asr_output
    ...
    def chunk_completed_sentence(self):
        ...
        logger.debug("COMPLETED SENTENCE: " +
                     " ".join(token.text for token in self.committed))
        ...
        for sent_text in sentence_texts:
            ...
            while token_index < len(tokens) and len(accumulated) < len(sent_text):
                ...
                accumulated = (accumulated + " " +
                               token.text).strip() if accumulated else token.text
    ...
    def concatenate_tokens(
        ...
        probability = sum(
            token.probability for token in tokens if token.probability) / len(tokens) if tokens else None
        if tokens:
