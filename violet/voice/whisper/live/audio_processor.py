...
class AudioProcessor:
    ...
    def __init__(self, **kwargs):
        ...
        # to deal with a potential little lag at the websocket initialization, this is now set in process_audio
        self.beg_loop = None
        ...
        while True:
            try:
                ...
                asr_internal_buffer_duration_s = len(
                    getattr(self.online, 'audio_buffer', [])) / self.online.SAMPLING_RATE
                transcription_lag_s = max(
                    0.0, time() - self.beg_loop - self.end_buffer)
                ...
                if new_tokens:
                    validated_text = self.sep.join(
                        [t.text for t in new_tokens])
                    if buffer_text.startswith(validated_text):
                        buffer_text = buffer_text[len(
                            validated_text):].lstrip()
    ...
    async def diarization_processor(self, diarization_obj):
        ...
        while True:
            try:
                ...
                if item is SENTINEL:
                    logger.debug(
                        "Diarization processor received sentinel. Finishing.")
        ...
        while True:
            try:
                ...
                for i, task in enumerate(tasks_to_monitor):
                    if task.done():
                        exc = task.exception()
                        task_name = task.get_name() if hasattr(
                            task, 'get_name') else f"Monitored Task {i}"
                        if exc:
                            logger.error(
                ...
                if ffmpeg_state == FFmpegState.FAILED:
                    ...
                    logger.warning(
                        "FFmpeg unexpectedly stopped, attempting restart")
    ...
    async def process_audio(self, message):
        ...
        if not message:
            logger.info(
                "Empty audio message received, initiating stop sequence.")
        ...
        if self.is_stopping:
            logger.warning(
                "AudioProcessor is stopping. Ignoring incoming audio.")
            return
