...
from violet.agent.app_constants import TEMPORARY_MESSAGE_LIMIT, GEMINI_MODELS, SKIP_META_MEMORY_MANAGER
from violet.constants import CHAINING_FOR_MEMORY_UPDATE
from violet.agent.app_utils import encode_image
...
class TemporaryMessageAccumulator:
    ...
    def __init__(self, client, timezone, upload_manager, message_queue,
        ...
        self.logger = logging.getLogger(
            f"violet.TemporaryMessageAccumulator.{model_name}")
    ...
    def add_message(self, full_message, timestamp, delete_after_upload=True, async_upload=True):
        ...
        if self.needs_upload and self.upload_manager is not None:
            if 'image_uris' in full_message and full_message['image_uris']:
                ...
                if async_upload:
                    image_file_ref_placeholders = [self.upload_manager.upload_file_async(
                        image_uri, timestamp) for image_uri in full_message['image_uris']]
                else:
                    image_file_ref_placeholders = [self.upload_manager.upload_file(
                        image_uri, timestamp) for image_uri in full_message['image_uris']]
            ...
            if 'voice_files' in full_message and full_message['voice_files']:
                ...
                for i, voice_file in enumerate(full_message['voice_files']):
                    # TODO replace voice module
                    # converted_segment = convert_base64_to_audio_segment(
                    #     voice_file)
                    converted_segment = None
                    if converted_segment is not None:
                        ...
                        continue
                audio_segment = None if len(
                    audio_segment) == 0 else audio_segment
                if audio_segment:
                    self.logger.info(
                        f"✅ Successfully processed {len(audio_segment)} voice segments")
                else:
                    self.logger.info(
                        "❌ No voice segments were successfully processed")
            else:
                audio_segment = None
            ...
            with self._temporary_messages_lock:
                ...
                total_images = sum(len(item.get('image_uris', []) or [])
                                   for _, item in self.temporary_messages)
                total_voice_segments = sum(len(item.get('audio_segments', []) or [
                ]) for _, item in self.temporary_messages)

            if delete_after_upload and full_message['image_uris']:
                threading.Thread(
                    target=self._cleanup_file_after_upload,
                    args=(full_message['image_uris'],
            ...
            with self._temporary_messages_lock:
                ...
                self.temporary_messages.append(
                    (timestamp, {
                        ...
                        'message': full_message['message'],
                        # Store delete flag for OpenAI models
    ...
    def get_recent_images_for_chat(self, current_timestamp):
        ...
        with self._temporary_messages_lock:
            ...
            recent_limit = min(self.temporary_message_limit,
                               len(self.temporary_messages))
    ...
    def absorb_content_into_memory(self, agent_states, ready_messages=None):
                            ...
                            if self.needs_upload and self.upload_manager is not None:
                                ...
                                if isinstance(file_ref, dict) and file_ref.get('pending'):
                                    ...
                                    if upload_status['status'] == 'completed':
                                        ...
                                        # Upload was cleaned up, treat as failed
                                        print(
                                            f"Skipping unknown/cleaned upload in absorb_content_into_memory")
        ...
        if voice_content:
            ...
            try:
                ...
                self.logger.info(
                    f"Created voice content folder: {voice_folder}")
                ...
                for i, audio_segment in enumerate(voice_content):
                    try:
                        ...
                        if hasattr(audio_segment, 'export'):
                            ...
                            self.logger.info(
                                f"Saved voice data {i+1} to {filepath}")
                    except Exception as e:
                        self.logger.error(
                            f"Failed to save voice segment {i+1}: {e}")
        ...
        message, user_message_added = self._add_user_conversation_to_message(
            message)
        ...
        if SKIP_META_MEMORY_MANAGER:
            ...
            self._send_to_memory_agents_separately(message, set(
                list(self.uri_to_create_time.keys())), agent_states)
        else:
            # Send to meta memory agent
            response, agent_type = self._send_to_meta_memory_agent(
                message, set(list(self.uri_to_create_time.keys())), agent_states)

        t2 = time.time()
        self.logger.info(
            f"Time taken to send to memory agents: {t2 - t1} seconds")
            ...
            if 'image_uris' in item and item['image_uris']:
                ...
                if sources and len(sources) == len(image_uris):
                    ...
                    for file_ref in image_uris:
                        images_by_source[generic_source].append(
                            (timestamp, file_ref))
                ...
                for timestamp, file_ref in source_images:
                    ...
                    if hasattr(file_ref, 'uri'):
                        ...
                        try:
                            ...
                            self.logger.error(
                                f"Failed to encode image {file_ref}: {e}")
    ...
    def _cleanup_processed_content(self, ready_to_process, user_message_added):
        ...
        if self.needs_upload and self.upload_manager is not None:
            ...
            for timestamp, item in ready_to_process:
                ...
                should_delete = item.get('delete_after_upload', True)
            ...
            while retry_count < max_retries:
                try:
                    if os.path.exists(image_path):
                        ...
                        self.logger.debug(
                            f"Deleted processed image file: {image_path}")
    ...
    def _cleanup_file_after_upload(self, filenames, placeholders):
        ...
        for filename, placeholder in zip(filenames, placeholders):
            placeholder_id = id(placeholder) if isinstance(
                placeholder, dict) else None
    ...
    def update_model(self, new_model_name):
        ...
        self.logger = logging.getLogger(
            f"violet.TemporaryMessageAccumulator.{new_model_name}")
        self.logger.setLevel(logging.INFO)
