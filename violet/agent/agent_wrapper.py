...
from violet.agent.temporary_message_accumulator import TemporaryMessageAccumulator
from violet.agent.upload_manager import UploadManager
from violet.agent.agent_states import AgentStates
from violet.agent.agent_configs import AGENT_CONFIGS
from violet.agent.app_constants import TEMPORARY_MESSAGE_LIMIT, MAXIMUM_NUM_IMAGES_IN_CLOUD, GEMINI_MODELS, OPENAI_MODELS, WITH_REFLEXION_AGENT, WITH_BACKGROUND_AGENT
from violet.schemas.violet_message import MessageType

from violet import create_client
from violet import LLMConfig, EmbeddingConfig
from violet.schemas.agent import AgentType
...
def get_image_mime_type(image_path):
    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            return format_to_mime.get(format_lower, 'image/jpeg')
...
class AgentWrapper():
    ...
    def __init__(self, agent_config_file, load_from=None):
        ...
        self.logger = logging.getLogger(
            f"violet.AgentWrapper.{self.agent_name}")
        ...
        if len(self.client.list_agents()) > 0:
            ...
            if self.agent_states.reflexion_agent_state is None:
                ...
                setattr(self.agent_states, 'reflexion_agent_state',
                        reflexion_agent_state)
            ...
            if self.agent_states.background_agent_state is None:
                ...
                setattr(self.agent_states, 'background_agent_state',
                ...
                if config['name'] == 'chat_agent':
                    ...
                    agent_state = self.client.create_agent(
                        ...
                        system=gpt_system.get_system_text(
                            'screen_monitor/' + config['name']) if self.is_screen_monitor else gpt_system.get_system_text('base/' + config['name'])
                    )
                else:
        ...
        # Initialize components that all violet models need
    ...
    def _restore_database_before_init(self, folder_path: str):
        ...
        try:
            ...
            if config_file.exists():
                ...
                if (folder / "violet_database.sql").exists() or (folder / "violet_database.dump").exists():
                    backup_type = 'postgresql'
                elif (folder / "sqlite.db").exists():
                    backup_type = 'sqlite'
                else:
                    raise ValueError(
                        f'No valid backup files found in {folder_path}')

            print(f"📁 Detected backup type: {backup_type}")

            # Handle PostgreSQL restoration
            if backup_type == 'postgresql':
                if not settings.violet_pg_uri_no_default:
                    raise ValueError(
                ...
                if settings.pg_uri:
                    ...
                    db_user = parsed.username or 'violet'
                    db_name = parsed.path.lstrip('/') or 'violet'
                else:
                    db_host = settings.pg_host or 'localhost'
                    db_port = settings.pg_port or 5432
                    db_user = settings.pg_user or 'violet'
                    db_name = settings.pg_db or 'violet'

                # Use compressed backup if available, otherwise SQL backup
                compressed_backup = folder / "violet_database.dump"
                sql_backup = folder / "violet_database.sql"
                ...
                if result_proc.returncode != 0:
                    ...
                    if "must be owner of extension vector" in stderr_text:
                        ...
                        raise RuntimeError(
                            f"PostgreSQL restore failed: {stderr_text}")
            ...
            # Handle SQLite restoration
                ...
                if not sqlite_backup.exists():
                    raise ValueError(
                        f'SQLite backup file not found in {folder_path}')

                sqlite_dest = Path.home() / ".violet" / "sqlite.db"
    ...
    def _process_existing_uploaded_files(self):
        ...
        uploaded_mappings = self.client.server.cloud_file_mapping_manager.list_files_with_status(
            status='uploaded')

        count = 0
        for mapping in uploaded_mappings:
            file_ref = [
                file for file in self.existing_files if file.name == mapping.cloud_file_id][0]
    ...
    def set_persona(self, persona_name):
        ...
        persona_block = [
            block for block in blocks if block.label == 'persona'][0]
    ...
    def get_persona_details(self) -> dict[str, str]:
        ...
        personas_dir = os.path.join(
            os.path.dirname(gpt_persona.__file__), "personas")
            ...
            if os.path.exists(personas_dir):
                for filename in os.listdir(personas_dir):
                    if filename.endswith('.txt'):
                        ...
                        try:
                            persona_text = gpt_persona.get_persona_text(
                                persona_name)
    ...
    def clear_old_screenshots(self):
        ...
        if len(self.uri_to_create_time) > MAXIMUM_NUM_IMAGES_IN_CLOUD:
            ...
            if self.google_client is not None:
                threading.Thread(target=self.delete_files, args=(
                    [x for x in file_names_to_delete], self.google_client)).start()
            else:
                self.logger.warning(
    ...
    def reflextion_on_memory(self):
        ...
        self.logger.info(
            "Reflexion process completed with actual agent actions.")
    ...
    def _call_agents_to_remove_redundancy(self):
        ...
        self.logger.info(
            "Calling episodic memory agent to remove redundancy...")
        try:
        ...
        self.logger.info(
            "Calling semantic memory agent to remove redundancy...")
        try:
        ...
        self.logger.info(
            "Calling resource memory agent to remove redundancy...")
        try:
        ...
        self.logger.info(
            "Calling procedural memory agent to remove redundancy...")
        try:
        ...
        self.logger.info(
            "Calling knowledge vault agent to remove redundancy...")
        try:
    ...
    def _call_reflexion_agent_for_conflicts(self):
        ...
        self.logger.info(
            "Calling reflexion agent to resolve memory conflicts...")
...
Please perform these actions and provide a comprehensive report of all conflict resolutions made."""
            ...
            self.logger.error(
                f"Error calling reflexion agent for conflicts: {e}")
            return f"Error: {e}"
...
Please perform this analysis and create new memories as appropriate. Provide a detailed report of patterns found and new memories created."""
            ...
            self.logger.error(
                f"Error calling reflexion agent for patterns: {e}")
            pattern_results['reflexion_patterns'] = f"Error: {e}"

        # Call semantic memory agent for new connections
        self.logger.info(
            "Calling semantic memory agent for new connections...")
        try:
            ...
            self.logger.error(
                f"Error calling semantic memory agent for patterns: {e}")
            pattern_results['semantic_connections'] = f"Error: {e}"

        # Call meta memory agent for high-level insights
        self.logger.info(
            "Calling meta memory agent for high-level insights...")
        try:
    ...
    def _remove_memory_redundancy(self):
        ...
        try:
            ...
            core_blocks = self.client.server.block_manager.get_blocks(
                self.client.user)
            redundancy_results['core'] = self._analyze_core_redundancy(
                core_blocks)
    ...
    def _analyze_memory_patterns(self):
        ...
        try:
            ...
            pattern_results['relationship_patterns'] = self._identify_relationship_patterns(
                all_memories)
    ...
    def _check_core_overlap(self, block1, block2):
        ...
        try:
            ...
            overlap_ratio = len(words1.intersection(
                words2)) / min(len(words1), len(words2))
    ...
    def save_agent(self, folder_path: str) -> dict:
        ...
        try:
            ...
            if settings.violet_pg_uri_no_default:
                ...
                backup_file = Path(folder_path) / "violet_database.sql"
                cmd = [
                    'pg_dump',
                    '-h', str(db_host),
                    '-p', str(db_port),
                ...
                try:
                    ...
                    self.logger.info(
                        f"✅ PostgreSQL backup created: {backup_file}")

                    # Also create a compressed backup for efficiency
                    compressed_backup = Path(
                        folder_path) / "violet_database.dump"
                    cmd_compressed = [
                        'pg_dump',
                        '-h', str(db_host),
                        '-p', str(db_port),
                    ...
                    config_dest = Path(folder_path) / "violet_config.yaml"
                    with open(config_dest, "w") as f:
                        yaml.dump(self.agent_config, f,
                                  default_flow_style=False, indent=2)
                    self.logger.info(
                        f"✅ Agent configuration saved: {config_dest}")
                ...
                self.logger.info(
                    f"Creating SQLite backup for agent in {folder_path}")
                sqlite_source = Path.home() / ".violet" / "sqlite.db"
    ...
    def get_database_info(self) -> dict:
        ...
        info = {
            'database_type': 'postgresql' if settings.violet_pg_uri_no_default else 'sqlite',
            'tables': {},
            'total_records': 0
        }
        ...
        try:
            if settings.violet_pg_uri_no_default:
                ...
                info['connection_details'] = {
                    ...
                    'user': settings.pg_user or 'violet',
                    'database': settings.pg_db or 'violet'
                }
                ...
                # SQLite info
                sqlite_path = Path.home() / ".violet" / "sqlite.db"
    ...
    def export_memories_to_csv(self, csv_file_path: str, include_embeddings: bool = False) -> dict:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Exported {len(episodic_memories)} episodic memories")
            ...
            try:
                ...
                self.logger.info(
                    f"Exported {len(semantic_memories)} semantic memories")
            ...
            try:
                ...
                self.logger.info(
                    f"Exported {len(resource_memories)} resource memories")
            ...
            try:
                ...
                self.logger.error(
                    f"Error exporting knowledge vault items: {e}")
            ...
            if all_memories:
                ...
                self.logger.info(
                    f"✅ Memory export completed: {result['message']}")
        ...
        try:
            ...
            with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                ...
                if total_exported > 0:
                    result['success'] = True
                    result[
                        'message'] = f'Successfully exported {total_exported} memories to {file_path} with {len(memory_types)} sheets'
                else:
                    # Still success even if no memories
                    result['success'] = True
                    result[
                        'message'] = f'No memories found to export, created empty Excel file at {file_path}'
