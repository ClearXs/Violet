...
import logging
from violet import LLMConfig
from violet.errors import ContextWindowExceededError, LLMError
from violet.functions.ast_parsers import coerce_dict_args_by_annotations, get_function_annotations_from_source
from violet.functions.functions import get_function_from_module
from violet.helpers import ToolRulesSolver
from violet.helpers.message_helpers import prepare_input_message_create
...
from violet.schemas.agent import AgentState, AgentStepResponse, UpdateAgent
...
from violet.schemas.memory import ContextWindowOverview, Memory
from violet.schemas.message import Message, MessageCreate
from violet.schemas.openai.chat_completion_request import Tool as ChatCompletionRequestTool
from violet.schemas.openai.chat_completion_response import ChatCompletionResponse
from violet.schemas.openai.chat_completion_response import Message as ChatCompletionMessage
from violet.schemas.openai.chat_completion_response import UsageStatistics
from violet.schemas.tool import Tool
from violet.schemas.tool_rule import TerminalToolRule
from violet.schemas.usage import VioletUsageStatistics
from violet.schemas.violet_message_content import TextContent, ImageContent, FileContent, CloudFileContent
from violet.services.agent_manager import AgentManager
from violet.services.block_manager import BlockManager
from violet.services.helpers.agent_manager_helper import check_supports_structured_output, compile_memory_metadata_block
...
class Agent(BaseAgent):
    def __init__(
        ...
        # in-memory representation of the agent state (read from multiple tables)
        ...
        assert isinstance(
            agent_state.memory, Memory), f"Memory object is not of type Memory: {type(agent_state.memory)}"
        # Hold a copy of the state that was used to init the agent
        self.agent_state = agent_state
        assert isinstance(self.agent_state.memory,
                          Memory), f"Memory object is not of type Memory: {type(self.agent_state.memory)}"

        self.user = user

        # Initialize logger early in constructor
        self.logger = logging.getLogger(f"violet.Agent.{agent_state.name}")
    ...
    def update_memory_if_changed(self, new_memory: Memory) -> bool:
        ...
        if self.agent_state.memory.compile() != new_memory.compile():
            ...
            self.agent_state.memory = Memory(
                blocks=[self.block_manager.get_block_by_id(
                    block.id, actor=self.user) for block in self.agent_state.memory.get_blocks()]
            )
    ...
    def execute_tool_and_persist_state(self, function_name: str, function_args: dict, target_violet_tool: Tool,
        ...
        try:
                    ...
                    for item in function_args[key]:
                        if 'occurred_at' in item:
                            item['occurred_at'] = convert_timezone_to_utc(
                                item['occurred_at'], self.user_manager.get_user_by_id(self.user.id).timezone)
            ...
            if function_name in ['search_in_memory', 'list_memory_within_timerange']:
                function_args['timezone_str'] = self.user_manager.get_user_by_id(
                    self.user.id).timezone

            if target_violet_tool.tool_type == ToolType.MIRIX_CORE:
                # base tools are allowed to access the `Agent` object and run on the database
                callable_func = get_function_from_module(
                    MIRIX_CORE_TOOL_MODULE_NAME, function_name)
                # need to attach self to arg since it's dynamically linked
            ...
            elif target_violet_tool.tool_type == ToolType.MIRIX_MEMORY_CORE:
                callable_func = get_function_from_module(
                ...
                raise ValueError(
                    f"Tool type {target_violet_tool.tool_type} not supported")

        except Exception as e:
            # Need to catch error here, or else trunction wont happen
            # TODO: modify to function execution error
            function_response = get_friendly_error_msg(
                function_name=function_name, exception_name=type(
                    e).__name__, exception_message=str(e)
            )
    ...
    def _get_ai_reply(
        ...
        agent_state_tool_jsons = [
            t.json_schema for t in self.agent_state.tools]
        ...
        if last_function_failed and self.tool_rules_solver.tool_call_history:
            allowed_functions = [f for f in allowed_functions if f["name"]
                                 != self.tool_rules_solver.tool_call_history[-1]]
        ...
        for attempt in range(1, empty_response_retry_limit + 1):
            try:
                ...
                if response.choices[0].finish_reason not in ["stop", "function_call", "tool_calls"]:
                    if response.choices[0].finish_reason == "length":
                        if attempt >= empty_response_retry_limit:
                            ...
                            delay = min(backoff_factor *
                                        (2 ** (attempt - 1)), max_delay)
                            self.logger.warning(
                ...
                if attempt >= empty_response_retry_limit:
                    self.logger.error(
                        f"Retry limit reached. Final error: {ve}")
                    log_telemetry(
                        self.logger, "_handle_ai_response finish ValueError")
                    raise Exception(
                ...
                if attempt >= empty_response_retry_limit:
                    self.logger.error(
                        f"Retry limit reached. Final error: {ke}")
                    log_telemetry(
                        self.logger, "_handle_ai_response finish KeyError")
                    raise Exception(
                ...
                if attempt >= empty_response_retry_limit:
                    self.logger.error(
                        f"Retry limit reached. Final error: {llm_error}")
                    log_telemetry(
                        self.logger, "_handle_ai_response finish LLMError")
                    log_telemetry(
                ...
                if attempt >= empty_response_retry_limit:
                    self.logger.error(
                        f"Retry limit reached. Final error: {ae}")
                    raise Exception(
                ...
                if attempt >= empty_response_retry_limit:
                    self.logger.error(
                        f"Retry limit reached. Final error: {he}")
                    raise Exception(
            ...
            if response.usage.total_tokens > self.agent_state.llm_config.context_window:
                ...
                log_telemetry(
                    self.logger, "_get_ai_reply summarize_messages_inplace")
        ...
        log_telemetry(
            self.logger, "_handle_ai_response finish catch-all exception")
        raise Exception("Retries exhausted and no valid response received.")

    def _handle_ai_response(
        self,
        input_message: Message,
        # TODO should we eventually move the Message creation outside of this function?
        ...
        if response_message.function_call or (response_message.tool_calls is not None and len(response_message.tool_calls) > 0):
            ...
            for tool_call_idx, tool_call in enumerate(response_message.tool_calls):
                ...
                target_violet_tool = None
                for t in self.agent_state.tools:
                    if t.name == function_name:
                        target_violet_tool = t

                if not target_violet_tool:
                ...
                if "inner_thoughts" in function_args:
                    response_message.content = function_args.pop(
                        "inner_thoughts")
                # The content if then internal monologue, not chat
                ...
                try:
                    ...
                    if function_name == 'send_message' or function_name == 'finish_memory_update':
                        assert tool_call_idx == len(
                            response_message.tool_calls) - 1, f"{function_name} must be the last tool call"
                    ...
                    return_char_limit = target_violet_tool.return_char_limit
                    ...
                    error_msg = get_friendly_error_msg(function_name=function_name, exception_name=type(
                        e).__name__, exception_message=str(e))
            ...
            if not overall_function_failed:
                ...
                if should_clear_history:
                    ...
                    in_context_messages = self.agent_manager.get_in_context_messages(
                        agent_id=self.agent_state.id, actor=self.user)
                    message_ids = [
                        message.id for message in in_context_messages]
                    ...
                    if self.agent_state.name == 'episodic_memory_agent':
                        ...
                        if memory_item:
                            ...
                            memory_item_str = ''
                            memory_item_str += '[Episodic Event ID]: ' + \
                                memory_item.id + '\n'
                            memory_item_str += '[Event Occurred At]: ' + \
                                memory_item.occurred_at.strftime(
                                    '%Y-%m-%d %H:%M:%S') + '\n'
                            memory_item_str += '[Summary]: ' + \
                                memory_item.summary + '\n'
                            memory_item_str += '[Details]: ' + \
                                memory_item.details + '\n'
                            memory_item_str += '[Tree Path]: ' + (' > '.join(
                                memory_item.tree_path) if memory_item.tree_path else 'N/A') + '\n'
                            memory_item_str += '[Last Modified]: ' + memory_item.last_modify['operation'] + \
                                ' at ' + \
                        ...
                        if memory_item:
                            memory_item = memory_item[0]
                            memory_item_str = ''
                            memory_item_str += '[Procedural Memory ID]: ' + \
                                memory_item.id + '\n'
                            memory_item_str += '[Entry Type]: ' + \
                                memory_item.entry_type + '\n'
                            memory_item_str += '[Summary]: ' + \
                                (memory_item.summary or 'N/A') + '\n'
                            memory_item_str += '[Steps]: ' + \
                                "; ".join(memory_item.steps) + '\n'
                            memory_item_str += '[Tree Path]: ' + (' > '.join(
                                memory_item.tree_path) if memory_item.tree_path else 'N/A') + '\n'
                            memory_item_str += '[Last Modified]: ' + memory_item.last_modify['operation'] + \
                                ' at ' + \
                        ...
                        if memory_item:
                            memory_item = memory_item[0]
                            memory_item_str = ''
                            memory_item_str += '[Resource Memory ID]: ' + \
                                memory_item.id + '\n'
                            memory_item_str += '[Title]: ' + \
                                memory_item.title + '\n'
                            memory_item_str += '[Summary]: ' + \
                                (memory_item.summary or 'N/A') + '\n'
                            memory_item_str += '[Resource Type]: ' + \
                                memory_item.resource_type + '\n'
                            memory_item_str += '[Content]: ' + \
                                memory_item.content + '\n'
                            memory_item_str += '[Tree Path]: ' + (' > '.join(
                                memory_item.tree_path) if memory_item.tree_path else 'N/A') + '\n'
                            memory_item_str += '[Last Modified]: ' + memory_item.last_modify['operation'] + \
                        ...
                        if memory_item:
                            memory_item = memory_item[0]
                            memory_item_str = ''
                            memory_item_str += '[Knowledge Vault ID]: ' + \
                                memory_item.id + '\n'
                            memory_item_str += '[Entry Type]: ' + \
                                memory_item.entry_type + '\n'
                            memory_item_str += '[Caption]: ' + \
                                memory_item.caption + '\n'
                            memory_item_str += '[Source]: ' + \
                                memory_item.source + '\n'
                            memory_item_str += '[Sensitivity]: ' + \
                                memory_item.sensitivity + '\n'
                            memory_item_str += '[Secret Value]: ' + \
                                memory_item.secret_value + '\n'
                            memory_item_str += '[Last Modified]: ' + memory_item.last_modify['operation'] + \
                                ' at ' + \
                                memory_item.last_modify['timestamp'].strftime(
                                    '%Y-%m-%d %H:%M:%S') + '\n'
                        ...
                        if memory_item:
                            memory_item = memory_item[0]
                            memory_item_str = ''
                            memory_item_str += '[Semantic Memory ID]: ' + \
                                memory_item.id + '\n'
                            memory_item_str += '[Name]: ' + \
                                memory_item.name + '\n'
                            memory_item_str += '[Summary]: ' + \
                                memory_item.summary + '\n'
                            memory_item_str += '[Details]: ' + \
                                (memory_item.details or 'N/A') + '\n'
                            memory_item_str += '[Source]: ' + \
                                (memory_item.source or 'N/A') + '\n'
                            memory_item_str += '[Tree Path]: ' + (' > '.join(
                                memory_item.tree_path) if memory_item.tree_path else 'N/A') + '\n'
                            memory_item_str += '[Last Modified]: ' + memory_item.last_modify['operation'] + \
                                ' at ' + \
                    ...
                    if memory_item_str:
                        ...
                        self.agent_manager.set_in_context_messages(
                            agent_id=self.agent_state.id, message_ids=message_ids, actor=self.user)

                        # delete the detached messages
                        deleted_count = self.message_manager.delete_detached_messages_for_agent(
                            agent_id=self.agent_state.id, actor=self.user)
    ...
    def step(
        ...
        **kwargs,
        ...
        message_objects = [prepare_input_message_create(
            m, self.agent_state.id, wrap_user_message=False, wrap_system_message=True) for m in input_messages]

        extra_message_objects = [prepare_input_message_create(
            m, self.agent_state.id, wrap_user_message=False, wrap_system_message=True) for m in extra_messages] if extra_messages is not None else None
        next_input_message = message_objects
        counter = 0
        total_usage = UsageStatistics()
        step_count = 0

        initial_message_count = len(self.agent_manager.get_in_context_messages(
            agent_id=self.agent_state.id, actor=self.user))
        ...
        if self.agent_state.name == 'reflexion_agent':
            ...
            self.agent_manager.set_in_context_messages(agent_id=self.agent_state.id, message_ids=[
                                                       message.id for message in in_context_messages], actor=self.user)
            ...
            if self.agent_state.name in ['meta_memory_agent', 'chat_agent'] and step_count == 0:
                ...
                try:
                    ...
                    if topics is not None:
                        ...
                        self.logger.warning(
                            "No topics extracted from screenshots")
        ...
        return VioletUsageStatistics(**total_usage.model_dump(), step_count=step_count)
    ...
    def build_system_prompt_with_memories(self, raw_system: str, topics: Optional[str] = None, retrieved_memories: Optional[dict] = None) -> Tuple[str, dict]:
        ...
        if key_words != '' and search_method == 'embedding':
            embedded_text = embedding_model(
                self.agent_state.embedding_config).get_text_embedding(key_words)
            embedded_text = np.array(embedded_text)
            embedded_text = np.pad(
                embedded_text, (0, MAX_EMBEDDING_DIM - embedded_text.shape[0]), mode="constant").tolist()
        else:
            embedded_text = None
        ...
        if self.agent_state.name == 'knowledge_vault' or 'knowledge_vault' not in retrieved_memories:
            if self.agent_state.name == 'knowledge_vault' or self.agent_state.name == 'reflexion_agent':
                current_knowledge_vault = self.knowledge_vault_manager.list_knowledge(
                    agent_state=self.agent_state, embedded_text=embedded_text, query=key_words, search_field='caption', search_method=search_method, limit=MAX_RETRIEVAL_LIMIT_IN_SYSTEM, timezone_str=timezone_str)
            else:
                current_knowledge_vault = self.knowledge_vault_manager.list_knowledge(agent_state=self.agent_state, embedded_text=embedded_text, query=key_words,
                                                                                      search_field='caption', search_method=search_method, limit=MAX_RETRIEVAL_LIMIT_IN_SYSTEM, timezone_str=timezone_str, sensitivity=['low', 'medium'])
        ...
        if self.agent_state.name == 'episodic_memory_agent' or 'episodic' not in retrieved_memories:
            current_episodic_memory = self.episodic_memory_manager.list_episodic_memory(
                agent_state=self.agent_state, limit=MAX_RETRIEVAL_LIMIT_IN_SYSTEM, timezone_str=timezone_str)
            episodic_memory = ''
            ...
            most_relevant_episodic_memory = self.episodic_memory_manager.list_episodic_memory(
                agent_state=self.agent_state, embedded_text=embedded_text, query=key_words, search_field='details', search_method=search_method, limit=MAX_RETRIEVAL_LIMIT_IN_SYSTEM, timezone_str=timezone_str)
        ...
        if self.agent_state.name == 'resource_memory_agent' or 'resource' not in retrieved_memories:
            current_resource_memory = self.resource_memory_manager.list_resources(
                agent_state=self.agent_state, query=key_words, embedded_text=embedded_text, search_field='summary', search_method=search_method, limit=MAX_RETRIEVAL_LIMIT_IN_SYSTEM, timezone_str=timezone_str)
            resource_memory = ''
        ...
        if self.agent_state.name == 'semantic_memory_agent' or 'semantic' not in retrieved_memories:
            current_semantic_memory = self.semantic_memory_manager.list_semantic_items(
                agent_state=self.agent_state, query=key_words, embedded_text=embedded_text, search_field="details", search_method=search_method, limit=MAX_RETRIEVAL_LIMIT_IN_SYSTEM, timezone_str=timezone_str)
            semantic_memory = ''
...
"""
        user_timezone_str = self.user_manager.get_user_by_id(
            self.user.id).timezone
        ...
        **kwargs,
        ...
        try:
            ...
            if extra_messages is not None:
                input_message_sequence = input_message_sequence[:initial_message_count] + \
                    extra_messages + \
            ...
            if self.agent_state.llm_config.context_window is None:
                ...
                self.agent_state.llm_config.context_window = (
                    LLM_MAX_TOKENS[self.model] if (
                        self.model is not None and self.model in LLM_MAX_TOKENS) else LLM_MAX_TOKENS["DEFAULT"]
                )
            ...
            if current_total_tokens > summarizer_settings.memory_warning_threshold * int(self.agent_state.llm_config.context_window):
                ...
                if not self.agent_alerted_about_memory_pressure:
                    ...
                    # it's up to the outer loop to handle this
            ...
            if is_context_overflow_error(e):
                ...
                if summarize_attempt_count <= summarizer_settings.max_summarizer_retries:
                    ...
                    token_counts = (
                        get_token_counts_for_messages(in_context_messages),)
    ...
    def step_user_message(self, user_message_str: str, **kwargs) -> AgentStepResponse:
        ...
        # Turn into a dict
        openai_message_dict = {
            "role": "user", "content": cleaned_user_message_text, "name": name}
    ...
    def summarize_messages_inplace(self):
        ...
        cutoff = calculate_summarizer_cutoff(
            in_context_messages=in_context_messages, token_counts=token_counts, logger=self.logger)
        ...
        summary = summarize_messages(
            agent_state=self.agent_state, message_sequence_to_summarize=message_sequence_to_summarize)
        ...
        remaining_message_count = 1 + \
            len(in_context_messages) - cutoff  # System + remaining
    ...
    def get_context_window(self) -> ContextWindowOverview:
        ...
        if agent_state_tool_jsons:
            available_functions_definitions = [ChatCompletionRequestTool(
                type="function", function=f) for f in agent_state_tool_jsons]
        ...
        num_tokens_used_total = (
            ...
            # metadata (statistics) about recall/archival
...
def save_agent(agent: Agent):
    ...
    assert isinstance(
        agent_state.memory, Memory), f"Memory is not a Memory object: {type(agent_state.memory)}"
...
def strip_name_field_from_user_message(user_message_text: str) -> Tuple[str, Optional[str]]:
    ...
    try:
        ...
        logger = logging.getLogger("violet.Agent.Utils")
