...
from violet.orm import Tool as ToolModel
from violet.orm.errors import NoResultFound
from violet.orm.sandbox_config import AgentEnvironmentVariable as AgentEnvironmentVariableModel
from violet.orm.sqlite_functions import adapt_array
from violet.schemas.agent import AgentState as PydanticAgentState
from violet.schemas.agent import AgentType, CreateAgent, UpdateAgent
...
from violet.schemas.tool_rule import ToolRule as PydanticToolRule
from violet.schemas.user import User as PydanticUser
from violet.services.block_manager import BlockManager
from violet.services.helpers.agent_manager_helper import (
...
class AgentManager:
    ...
    def create_agent(
        ...
        if agent_create.agent_type == AgentType.reflexion_agent:
            tool_names.extend(SEARCH_MEMORY_TOOLS +
                              CHAT_AGENT_TOOLS + UNIVERSAL_MEMORY_TOOLS)
    ...
    def update_agent_tools_and_system_prompts(
        ...
        new_tool_names = [
            tool_name for tool_name in tool_names if tool_name not in existing_tool_names]
        tool_names_to_remove = [
            tool_name for tool_name in existing_tool_names if tool_name not in tool_names]
        ...
        if len(new_tool_names) > 0 or len(tool_names_to_remove) > 0:
            self.update_agent(agent_id=agent_id, agent_update=UpdateAgent(
                tool_ids=tool_ids), actor=actor)
    ...
    def _generate_initial_message_sequence(
        ...
        if supplied_initial_message_sequence is not None:
            ...
            init_messages.extend(
                package_initial_message_sequence(
                    agent_state.id, supplied_initial_message_sequence, agent_state.llm_config.model, actor)
            )
        else:
            init_messages = [
                PydanticMessage.dict_to_message(
                    agent_id=agent_state.id, model=agent_state.llm_config.model, openai_message_dict=msg)
    ...
    def _create_agent(
        ...
        with self.session_maker() as session:
            ...
            _process_relationship(session, new_agent,
                                  "tools", ToolModel, tool_ids, replace=True)
            _process_relationship(
    ...
    def update_system_prompt(self, agent_id: str, system_prompt: str, actor: PydanticUser) -> PydanticAgentState:
        agent_state = self.update_agent(
            agent_id=agent_id, agent_update=UpdateAgent(system=system_prompt), actor=actor)
        # Rebuild the system prompt if it's different
        agent_state = self.rebuild_system_prompt(
            agent_id=agent_state.id, system_prompt=system_prompt, actor=actor, force=True)
        return agent_state
    ...
    def _update_agent(self, agent_id: str, agent_update: UpdateAgent, actor: PydanticUser) -> PydanticAgentState:
        ...
        with self.session_maker() as session:
            ...
            scalar_fields = {"name", "system", "topic", "llm_config", "embedding_config",
                             "message_ids", "tool_rules", "description", "metadata_"}
            ...
            if agent_update.tool_ids is not None:
                _process_relationship(
                    session, agent, "tools", ToolModel, agent_update.tool_ids, replace=True)
            if agent_update.block_ids is not None:
                _process_relationship(
                    session, agent, "core_memory", BlockModel, agent_update.block_ids, replace=True)
    ...
    def get_agent_by_name(self, agent_name: str, actor: PydanticUser) -> PydanticAgentState:
        ...
        with self.session_maker() as session:
            agent = AgentModel.read(
                db_session=session, name=agent_name, actor=actor)
            return agent.to_pydantic()
    ...
    def get_in_context_messages(self, agent_id: str, actor: PydanticUser) -> List[PydanticMessage]:
        message_ids = self.get_agent_by_id(
            agent_id=agent_id, actor=actor).message_ids
    ...
    def rebuild_system_prompt(self, agent_id: str, system_prompt: str, actor: PydanticUser, force=False) -> PydanticAgentState:
        ...
        message_ids = [message.id] + \
            agent_state.message_ids[1:]  # swap index 0 (system)
    ...
    def trim_older_in_context_messages(self, num: int, agent_id: str, actor: PydanticUser) -> PydanticAgentState:
        message_ids = self.get_agent_by_id(
            agent_id=agent_id, actor=actor).message_ids
        new_messages = [message_ids[0]] + \
            message_ids[num:]  # 0 is system message
    ...
    def prepend_to_in_context_messages(self, messages: List[PydanticMessage], agent_id: str, actor: PydanticUser) -> PydanticAgentState:
        ...
        message_ids = [message_ids[0]] + \
            [m.id for m in new_messages] + message_ids[1:]
        return self.set_in_context_messages(agent_id=agent_id, message_ids=message_ids, actor=actor)
    ...
    def append_to_in_context_messages(self, messages: List[PydanticMessage], agent_id: str, actor: PydanticUser) -> PydanticAgentState:
        messages = self.message_manager.create_many_messages(
            messages, actor=actor)
        message_ids = self.get_agent_by_id(
            agent_id=agent_id, actor=actor).message_ids or []
    ...
    def update_block_with_label(
        ...
        with self.session_maker() as session:
            ...
            agent.core_memory = [
                b for b in agent.core_memory if b.label != block_label]
    ...
    def detach_block(
        ...
        with self.session_maker() as session:
            ...
            agent.core_memory = [
                b for b in agent.core_memory if b.id != block_id]
    ...
    def detach_tool(self, agent_id: str, tool_id: str, actor: PydanticUser) -> PydanticAgentState:
        ...
        with self.session_maker() as session:
            ...
            remaining_tools = [
                tool for tool in agent.tools if tool.id != tool_id]
