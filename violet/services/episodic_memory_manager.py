...
from violet.orm.episodic_memory import EpisodicEvent
...
class EpisodicMemoryManager:
        ...
        with self.session_maker() as session:
            try:
                episodic_memory_item = EpisodicEvent.read(
                    db_session=session, identifier=episodic_memory_id)
    ...
    def get_most_recently_updated_event(self, organization_id: Optional[str] = None, timezone_str: str = None) -> Optional[PydanticEpisodicEvent]:
        ...
        with self.session_maker() as session:
            ...
            query = select(EpisodicEvent).order_by(
                cast(text("episodic_memory.last_modify ->> 'timestamp'"),
                     DateTime).desc()
            )
    ...
    def delete_event_by_id(self, id: str) -> None:
        ...
        with self.session_maker() as session:
            try:
                episodic_memory_item = EpisodicEvent.read(
                    db_session=session, identifier=id)
        ...
        try:
            ...
            event = self.create_episodic_memory(
                PydanticEpisodicEvent(
                    ...
                    last_modify={"timestamp": datetime.now(
                        dt.timezone.utc).isoformat(), "operation": "created"},
                )
            )
    ...
    def list_episodic_memory(self,
        ...
        with self.session_maker() as session:
            ...
            if query == '':
                query_stmt = select(EpisodicEvent).order_by(
                    EpisodicEvent.occurred_at.desc())
                if limit:
                ...
                if search_method == 'embedding':
                    ...
                    if settings.violet_pg_uri_no_default:
                        ...
                        top_events = [event for score,
                                      event in scored_events[:limit]]
    ...
    def _postgresql_fulltext_search(self, session, base_query, query_text, search_field, limit):
        ...
        try:
            ...
            fallback_field = getattr(EpisodicEvent, search_field) if search_field and hasattr(
                EpisodicEvent, search_field) else EpisodicEvent.summary
    ...
    def update_event(self,
        ...
        with self.session_maker() as session:
            ...
            selected_event = EpisodicEvent.read(
                db_session=session, identifier=event_id)
