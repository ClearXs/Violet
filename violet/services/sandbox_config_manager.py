...
from violet.constants import VIOLET_DIR_TOOL_SANDBOX
from violet.log import get_logger
from violet.orm.errors import NoResultFound
from violet.orm.sandbox_config import SandboxConfig as SandboxConfigModel
from violet.orm.sandbox_config import SandboxEnvironmentVariable as SandboxEnvVarModel
from violet.schemas.environment_variables import SandboxEnvironmentVariable as PydanticEnvVar
from violet.schemas.environment_variables import SandboxEnvironmentVariableCreate, SandboxEnvironmentVariableUpdate
from violet.schemas.sandbox_config import LocalSandboxConfig
...
class SandboxConfigManager:
    ...
    def get_or_create_default_sandbox_config(self, sandbox_type: SandboxType, actor: PydanticUser) -> PydanticSandboxConfig:
        sandbox_config = self.get_sandbox_config_by_type(
            sandbox_type, actor=actor)
        if not sandbox_config:
            ...
            if sandbox_type == SandboxType.E2B:
                ...
                default_local_sandbox_path = VIOLET_DIR_TOOL_SANDBOX
                default_config = LocalSandboxConfig(
                    sandbox_dir=default_local_sandbox_path).model_dump(exclude_none=True)
    ...
    def create_or_update_sandbox_config(self, sandbox_config_create: SandboxConfigCreate, actor: PydanticUser) -> PydanticSandboxConfig:
        ...
        db_sandbox = self.get_sandbox_config_by_type(
            sandbox_config.type, actor=actor)
        if db_sandbox:
        ...
        if db_env_var:
            ...
            if update_data:
                db_env_var = self.update_sandbox_env_var(
                    db_env_var.id, SandboxEnvironmentVariableUpdate(**update_data), actor)
            else:
                printd(
    ...
    def get_sandbox_env_vars_as_dict(
        ...
        env_vars = self.list_sandbox_env_vars(
            sandbox_config_id, actor, cursor, limit)
        result = {}
