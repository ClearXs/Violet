...
from violet.schemas.sandbox_config import SandboxConfig, SandboxRunResult, SandboxType
...
class ToolExecutionSandbox:
    ...
    LOCAL_SANDBOX_RESULT_START_MARKER = str(uuid.uuid5(
        NAMESPACE, "local-sandbox-result-start-marker"))
    LOCAL_SANDBOX_RESULT_END_MARKER = str(uuid.uuid5(
        NAMESPACE, "local-sandbox-result-end-marker"))
    ...
    def run_local_dir_sandbox_venv(self, sbx_config: SandboxConfig, env: Dict[str, str], temp_file_path: str) -> SandboxRunResult:
        ...
        venv_path = os.path.join(
            local_configs.sandbox_dir, local_configs.venv_name)
        ...
        try:
            ...
            func_result, stdout = self.parse_out_function_results_markers(
                result.stdout)
            ...
            logger.error(
                f"Executing tool {self.tool_name} has process error: {e}")
    ...
    def run_local_dir_sandbox_runpy(self, sbx_config: SandboxConfig, env: Dict[str, str], temp_file_path: str) -> SandboxRunResult:
        ...
        )] if captured_stdout.getvalue() else []
        stderr_output = [captured_stderr.getvalue(
        )] if captured_stderr.getvalue() else []
    ...
    def parse_out_function_results_markers(self, text: str):
        ...
        start_index = text.index(
            self.LOCAL_SANDBOX_RESULT_START_MARKER) + marker_len
        ...
        try:
            ...
            if os.path.isfile(requirements_txt_path):
                ...
                subprocess.run([pip_path, "install", "-r",
                               requirements_txt_path], env=env, check=True)
                logger.info(
            ...
            logger.error(
                f"Error while setting up the virtual environment: {e}")
            raise RuntimeError(
                f"Failed to set up the virtual environment: {e}")
    ...
    def run_e2b_sandbox(self, agent_state: Optional[AgentState] = None, additional_env_vars: Optional[Dict] = None) -> SandboxRunResult:
        ...
        if not sbx or self.force_recreate:
            if not sbx:
                ...
                logger.info(
                    f"Force recreated e2b sandbox with state: {sbx_config}")
            sbx = self.create_e2b_sandbox_with_metadata_hash(
                sandbox_config=sbx_config)
        ...
        if execution.results:
            ...
            raise ValueError(
                f"Tool {self.tool_name} returned execution with None")
    ...
    def parse_exception_from_e2b_execution(self, e2b_execution: "Execution") -> Exception:
        builtins_dict = __builtins__ if isinstance(
        ...
        exception_class = builtins_dict.get(
            e2b_execution.error.name, Exception)
    ...
    def create_e2b_sandbox_with_metadata_hash(self, sandbox_config: SandboxConfig) -> "Sandbox":
        ...
        if e2b_config.template:
            sbx = Sandbox(sandbox_config.get_e2b_config().template, metadata={
                          self.METADATA_CONFIG_STATE_KEY: state_hash})
        else:
            # no template
            sbx = Sandbox(metadata={self.METADATA_CONFIG_STATE_KEY: state_hash},
    ...
    def _convert_param_to_value(self, param_type: str, raw_value: str) -> str:
        ...
        if param_type == "string":
            ...
            raise TypeError(
                f"Unsupported type: {param_type}, raw_value={raw_value}")
        return str(value)
