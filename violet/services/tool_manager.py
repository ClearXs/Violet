...
from violet.functions.functions import derive_openai_json_schema, load_function_set
from violet.orm.enums import ToolType

# TODO: Remove this once we translate all of these to the ORM
from violet.orm.errors import NoResultFound
from violet.orm.tool import Tool as ToolModel
...
class ToolManager:
    ...
    def create_or_update_tool(self, pydantic_tool: PydanticTool, actor: PydanticUser) -> PydanticTool:
        ...
        if tool:
            ...
            if update_data:
                self.update_tool_by_id(
                    tool.id, ToolUpdate(**update_data), actor)
            else:
                printd(
    ...
    def create_tool(self, pydantic_tool: PydanticTool, actor: PydanticUser) -> PydanticTool:
        ...
        with self.session_maker() as session:
            ...
            # Re-raise other database-related errors
    ...
    def get_tool_by_id(self, tool_id: str, actor: PydanticUser) -> PydanticTool:
        ...
        with self.session_maker() as session:
            ...
            tool = ToolModel.read(db_session=session,
                                  identifier=tool_id, actor=actor)
    ...
    def upsert_base_tools(self, actor: PydanticUser) -> List[PydanticTool]:
        ...
        for module_name in module_names:
            full_module_name = f"violet.functions.function_sets.{module_name}"
            try:
