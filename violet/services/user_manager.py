...
from violet.orm.user import User as UserModel
...
class UserManager:
    ...
    def create_default_user(self, org_id: str = OrganizationManager.DEFAULT_ORG_ID) -> PydanticUser:
        ...
        with self.session_maker() as session:
            ...
            try:
                user = UserModel.read(db_session=session,
                                      identifier=self.DEFAULT_USER_ID)
            except NoResultFound:
                # If it doesn't exist, make it
                user = UserModel(id=self.DEFAULT_USER_ID, name=self.DEFAULT_USER_NAME,
                                 timezone=self.DEFAULT_TIME_ZONE, organization_id=org_id)
                user.create(session)
