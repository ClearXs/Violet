...
from violet import system
from violet.constants import IN_CONTEXT_MEMORY_KEYWORD, STRUCTURED_OUTPUT_MODELS
from violet.helpers import ToolRulesSolver
from violet.orm.agent import Agent as AgentModel
from violet.orm.agents_tags import AgentsTags
from violet.orm.errors import NoResultFound
from violet.prompts import gpt_system
from violet.schemas.agent import AgentState, AgentType
...
def _process_relationship(
    ...
    found_items = session.query(model_class).filter(
        model_class.id.in_(item_ids)).all()

    # Validate all items are found if allow_partial is False
    if not allow_partial and len(found_items) != len(item_ids):
        missing = set(item_ids) - {item.id for item in found_items}
        raise NoResultFound(
            f"Items not found in {relationship_name}: {missing}")
    ...
    if replace:
        ...
        new_items = [
            item for item in found_items if item.id not in current_ids]
...
def _process_tags(agent: AgentModel, tags: List[str], replace=True):
    ...
    if replace:
        ...
        agent.tags.extend(
            [tag for tag in new_tags if tag.tag not in existing_tags])
...
def compile_system_message(
    ...
    # TODO move this inside of BaseMemory?
    in_context_memory_last_edit: datetime.datetime,
...
def package_initial_message_sequence(
    ...
    for message_create in initial_message_sequence:
        ...
        init_messages.append(
            Message(role=message_create.role, text=packed_message,
                    organization_id=actor.organization_id, agent_id=agent_id, model=model)
        )
