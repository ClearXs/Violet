...
class ResourceMemoryManager:
    ...
    def _postgresql_fulltext_search(self, session, base_query, query_text, search_field, limit):
        ...
        try:
            ...
            fallback_field = getattr(ResourceMemoryItem, search_field) if search_field and hasattr(
                ResourceMemoryItem, search_field) else ResourceMemoryItem.title
    ...
    def get_most_recently_updated_item(self, organization_id: Optional[str] = None, timezone_str: str = None) -> Optional[PydanticResourceMemoryItem]:
        ...
        with self.session_maker() as session:
            ...
            query = select(ResourceMemoryItem).order_by(
                cast(text("resource_memory.last_modify ->> 'timestamp'"),
                     DateTime).desc()
            )
        ...
        with self.session_maker() as session:
            ...
            if query == '':
                ...
                query_stmt = select(ResourceMemoryItem).order_by(
                    cast(
                        text("resource_memory.last_modify ->> 'timestamp'"), DateTime).desc()
                )
                if limit:
            ...
            base_query = select(
                ...
                ResourceMemoryItem.summary_embedding.label(
                    "summary_embedding"),
            ...
            if search_method == 'string_match':
                main_query = base_query.where(func.lower(
                    getattr(ResourceMemoryItem, search_field)).contains(query.lower()))
                ...
                main_query = build_query(
                    ...
                    search_field=eval("ResourceMemoryItem." +
                                      search_field + "_embedding"),
                ...
                raise NotImplementedError(
                    "Fuzzy matching is not implemented yet.")
    ...
    def delete_resource_by_id(self, resource_id: str) -> None:
        ...
        with self.session_maker() as session:
            try:
                item = ResourceMemoryItem.read(
                    db_session=session, identifier=resource_id)
