...
from violet.orm.semantic_memory import SemanticMemoryItem
...
class SemanticMemoryManager:
    ...
    def _postgresql_fulltext_search(self, session, base_query, query_text, search_field, limit):
        ...
        try:
            ...
            if len(results) >= min(limit or 10, 10):
                ...
                for row in results:
                    ...
                    embedding_fields = ['name_embedding',
        ...
        try:
            ...
            fallback_field = getattr(SemanticMemoryItem, search_field) if search_field and hasattr(
                SemanticMemoryItem, search_field) else SemanticMemoryItem.name
            ...
            semantic_items = [SemanticMemoryItem(
        ...
        with self.session_maker() as session:
            try:
                semantic_memory_item = SemanticMemoryItem.read(
                    db_session=session, identifier=semantic_memory_id)
    ...
    def get_most_recently_updated_item(self, organization_id: Optional[str] = None, timezone_str: str = None) -> Optional[PydanticSemanticMemoryItem]:
        ...
        with self.session_maker() as session:
            ...
            query = select(SemanticMemoryItem).order_by(
                cast(text("semantic_memory.last_modify ->> 'timestamp'"),
                     DateTime).desc()
            )
        ...
        with self.session_maker() as session:
            ...
            if query == '':
                ...
                query_stmt = select(SemanticMemoryItem).order_by(
                    cast(
                        text("semantic_memory.last_modify ->> 'timestamp'"), DateTime).desc()
                )
                if limit:
                ...
                base_query = select(
                    ...
                    SemanticMemoryItem.summary_embedding.label(
                        "summary_embedding"),
                    SemanticMemoryItem.details_embedding.label(
                        "details_embedding"),
                    SemanticMemoryItem.embedding_config.label(
                        "embedding_config"),
                    SemanticMemoryItem.organization_id.label(
                        "organization_id"),
                ...
                if search_method == 'embedding':
                    ...
                    main_query = base_query.where(func.lower(
                        search_field).contains(query.lower()))
                    ...
                    if settings.violet_pg_uri_no_default:
                        ...
                        for item in all_items:
                            ...
                            if search_field and hasattr(item, search_field):
                                text_to_search = getattr(
                                    item, search_field) or ""
                            else:
