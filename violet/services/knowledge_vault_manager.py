...
class KnowledgeVaultManager:
    ...
    def _postgresql_fulltext_search(self, session, base_query, query_text, search_field, limit, sensitivity=None):
        ...
        try:
            ...
            fallback_field = getattr(KnowledgeVaultItem, search_field) if search_field and hasattr(
                KnowledgeVaultItem, search_field) else KnowledgeVaultItem.caption
    ...
    def get_most_recently_updated_item(self, organization_id: Optional[str] = None, timezone_str: str = None) -> Optional[PydanticKnowledgeVaultItem]:
        ...
        with self.session_maker() as session:
            ...
            query = select(KnowledgeVaultItem).order_by(
                cast(text("knowledge_vault.last_modify ->> 'timestamp'"),
                     DateTime).desc()
            )
        ...
        with self.session_maker() as session:
            ...
            if query == '':
                ...
                query_stmt = select(KnowledgeVaultItem).order_by(
                    cast(
                        text("knowledge_vault.last_modify ->> 'timestamp'"), DateTime).desc()
                )
                ...
                base_query = select(
                    ...
                    KnowledgeVaultItem.organization_id.label(
                        "organization_id"),
                ...
                if search_method == 'embedding':
                    ...
                    main_query = base_query.where(func.lower(
                        search_field).contains(func.lower(query)))
    ...
    def delete_knowledge_by_id(self, knowledge_vault_item_id: str) -> None:
        ...
        with self.session_maker() as session:
            try:
                item = KnowledgeVaultItem.read(
                    db_session=session, identifier=knowledge_vault_item_id)
