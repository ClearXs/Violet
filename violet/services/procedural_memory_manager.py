...
from violet.orm.procedural_memory import ProceduralMemoryItem
from violet.schemas.user import User as PydanticUser
from violet.schemas.procedural_memory import (
...
class ProceduralMemoryManager:
    ...
    def _postgresql_fulltext_search(self, session, base_query, query_text, search_field, limit):
        ...
        query_words = [word.strip()
                       for word in cleaned_query.split() if word.strip()]
        if not query_words:
            return []
        ...
        if len(tsquery_parts) > 1:
            tsquery_string_and = " & ".join(
                tsquery_parts)  # AND logic for precision
            tsquery_string_or = " | ".join(
                tsquery_parts)   # OR logic for recall
        else:
        ...
        try:
            ...
            if len(results) >= min(limit or 10, 10):
                ...
                for row in results:
                    ...
                    json_fields = ['last_modify',
                                   'metadata_', 'embedding_config']
                    ...
                    for field in embedding_fields:
                        if field in data and data[field] is not None:
                            data[field] = self._parse_embedding_field(
                                data[field])
        ...
        try:
            ...
            fallback_field = getattr(ProceduralMemoryItem, search_field) if search_field and hasattr(
                ProceduralMemoryItem, search_field) else ProceduralMemoryItem.summary
    ...
    def get_most_recently_updated_item(self, organization_id: Optional[str] = None, timezone_str: str = None) -> Optional[PydanticProceduralMemoryItem]:
        ...
        with self.session_maker() as session:
            ...
            query = select(ProceduralMemoryItem).order_by(
                cast(text("procedural_memory.last_modify ->> 'timestamp'"),
                     DateTime).desc()
            )
    ...
    def list_procedures(self,
        ...
        with self.session_maker() as session:
            ...
            if query == '':
                query_stmt = select(ProceduralMemoryItem).order_by(
                ...
                base_query = select(
                    ...
                    ProceduralMemoryItem.steps_embedding.label(
                        "steps_embedding"),
                    ProceduralMemoryItem.summary_embedding.label(
                        "summary_embedding"),
                    ProceduralMemoryItem.embedding_config.label(
                        "embedding_config"),
                    ProceduralMemoryItem.organization_id.label(
                        "organization_id"),
                ...
                if search_method == 'embedding':
                    ...
                    if search_field == 'steps':
                        ...
                        if settings.violet_pg_uri_no_default:
                            ...
                            search_condition = text(
                                "lower(steps) LIKE lower(:query)")
                        main_query = base_query.where(
                            search_condition).params(query=f'%{query}%')
                    else:
                        search_field_obj = eval(
                            "ProceduralMemoryItem." + search_field)
                        main_query = base_query.where(func.lower(
                            search_field_obj).contains(query.lower()))
                    ...
                    if settings.violet_pg_uri_no_default:
                        ...
                        for item in all_items:
                            ...
                            if search_field == 'summary':
                                ...
                                if isinstance(item.steps, list):
                                    ...
                                    texts.append(str(item.steps)
                                                 if item.steps else "")
                                text_to_search = ' '.join(texts)

                            # Preprocess the text into tokens
                            tokens = self._preprocess_text_for_bm25(
                                text_to_search)
                        ...
                        top_items = [item for score,
                                     item in scored_items[:limit]]
    ...
    def delete_procedure_by_id(self, procedure_id: str) -> None:
        ...
        with self.session_maker() as session:
            try:
                item = ProceduralMemoryItem.read(
                    db_session=session, identifier=procedure_id)
