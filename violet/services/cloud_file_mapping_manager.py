...
from violet.schemas.cloud_file_mapping import CloudFileMapping as PydanticCloudFileMapping
    ...
    def list_files_with_status(self, status):
        ...
        with self.session_maker() as session:
            ...
            stmt = select(CloudFileMapping).where(
                CloudFileMapping.status == status).order_by(CloudFileMapping.timestamp.asc())
