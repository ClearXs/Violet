...
from violet.schemas.block import Block, BlockUpdate, CreateBlock, Human, Persona
...
from violet.schemas.file import FileMetadata
...
from violet.schemas.violet_request import VioletRequest, VioletStreamingRequest
from violet.schemas.violet_response import VioletResponse, VioletStreamingResponse
from violet.schemas.llm_config import LLMConfig
from violet.schemas.memory import ArchivalMemorySummary, ChatMemory, CreateArchivalMemory, Memory, RecallMemorySummary
from violet.schemas.message import Message, MessageCreate, MessageUpdate
from violet.schemas.openai.chat_completion_response import UsageStatistics
from violet.schemas.openai.chat_completions import ToolCall
from violet.schemas.organization import Organization
from violet.schemas.sandbox_config import E2BSandboxConfig, LocalSandboxConfig, SandboxConfig, SandboxConfigCreate, SandboxConfigUpdate
...
class Client(object):
    ...
    def create_agent(
        ...
        metadata: Optional[Dict] = {
            "human:": DEFAULT_HUMAN, "persona": DEFAULT_PERSONA},
    ...
    def user_message(self, agent_id: str, message: str) -> VioletResponse:
        raise NotImplementedError
...
class LocalClient(Client):
    ...
    def __init__(
        ...
        self.organization = self.server.get_organization_or_default(
            self.org_id)
    ...
    def _save_file_from_path(self, file_path: str) -> FileMetadata:
        ...
        try:
            ...
            raise ValueError(
                f"Failed to save file from path {file_path}: {str(e)}")
    ...
    def agent_exists(self, agent_id: Optional[str] = None, agent_name: Optional[str] = None) -> bool:
        ...
        if agent_id and agent_name:
            raise ValueError(
                f"Only one of agent_id or agent_name can be provided")
    ...
    def create_agent(
        ...
        tool_ids += [self.server.tool_manager.get_tool_by_name(
            tool_name=name, actor=self.server.user_manager.get_user_by_id(self.user.id)).id for name in tool_names]
    ...
    def add_tool_to_agent(self, agent_id: str, tool_id: str):
        ...
        agent_state = self.server.agent_manager.attach_tool(
            agent_id=agent_id, tool_id=tool_id, actor=self.server.user_manager.get_user_by_id(self.user.id))
        return agent_state
    ...
    def delete_agent(self, agent_id: str):
        ...
        self.server.agent_manager.delete_agent(
            agent_id=agent_id, actor=self.server.user_manager.get_user_by_id(self.user.id))
    ...
    def update_in_context_memory(self, agent_id: str, section: str, value: Union[List[str], str]) -> Memory:
        ...
        memory = self.server.update_agent_core_memory(
            agent_id=agent_id, label=section, value=value, actor=self.server.user_manager.get_user_by_id(self.user.id))
        return memory
    ...
    def send_messages(
        ...
        Returns:
            response (VioletResponse): Response from the agent
        """
        self.interface.clear()
        ...
        # format messages
        return VioletResponse(messages=messages, usage=usage)
    ...
    def send_message(
        ...
        if isinstance(message, str):
            ...
            input_messages = [MessageCreate(
                role=MessageRole(role), content=content, name=name)]
            ...
            def convert_message(m):
                if m['type'] == 'text':
                    ...
                    if url.startswith('data:'):
                        ...
                        file_metadata = self._save_image_from_base64(
                            url, detail)
                    else:
                        # HTTP URL - just create FileMetadata without downloading
                        file_metadata = self._create_file_metadata_from_url(
                            url, detail)
                    ...
                    if file_type.startswith('image/'):
                        # Handle as image
                        file_metadata = self._save_image_from_file_uri(
                            file_path)
                        return ImageContent(
                            type=MessageContentType.image_url,
                            image_id=file_metadata.id,
                    ...
                    file_uri = m.get(
                        'google_cloud_file_uri') or m.get('file_uri')

                    file_metadata = self._save_image_from_google_cloud_uri(
                        file_uri)
            ...
            if extra_messages is not None:
                extra_messages = [MessageCreate(role=MessageRole(role), content=[
                                                convert_message(m) for m in extra_messages], name=name)]
        ...
        violet_messages = []
        for m in messages:
            violet_messages += m.to_violet_message()

        return VioletResponse(messages=violet_messages, usage=usage)
    ...
    def run_command(self, agent_id: str, command: str) -> VioletResponse:
        """
        ...
        Returns:
            VioletResponse: The response from the agent
        ...
        return VioletResponse(messages=self.interface.to_list(), usage=usage)
    ...
    def get_block_id(self, name: str, label: str) -> str:
        block = self.server.block_manager.get_blocks(actor=self.server.user_manager.get_user_by_id(
            self.user.id), template_name=name, label=label, is_template=True)
        if not block:
            return None
        return block[0].id
    ...
    def get_persona_id(self, name: str) -> str:
        ...
        Returns:
            id (str): ID of the persona block
        """
        persona = self.server.block_manager.get_blocks(actor=self.server.user_manager.get_user_by_id(
            self.user.id), template_name=name, label="persona", is_template=True)
        if not persona:
            return None
        return persona[0].id
    ...
    def get_human_id(self, name: str) -> str:
        ...
        Returns:
            id (str): ID of the human block
        """
        human = self.server.block_manager.get_blocks(actor=self.server.user_manager.get_user_by_id(
            self.user.id), template_name=name, label="human", is_template=True)
        if not human:
            return None
        return human[0].id
    ...
    def update_tool(
        ...
        update_data = {key: value for key,
                       value in update_data.items() if value is not None}
    ...
    def create_block(
        ...
        """
        block = Block(label=label, template_name=template_name,
                      value=value, is_template=is_template, limit=limit)
        # if limit:
        #     block.limit = limit
    ...
    def update_block(self, block_id: str, name: Optional[str] = None, text: Optional[str] = None, limit: Optional[int] = None) -> Block:
        ...
        return self.server.block_manager.update_block(
            block_id=block_id,
            block_update=BlockUpdate(template_name=name, value=text,
    ...
    def update_sandbox_config(self, sandbox_config_id: str, config: Union[LocalSandboxConfig, E2BSandboxConfig]) -> SandboxConfig:
        ...
        return self.server.sandbox_config_manager.update_sandbox_config(
            sandbox_config_id=sandbox_config_id, sandbox_update=sandbox_update, actor=self.server.user_manager.get_user_by_id(
                self.user.id)
        )
    ...
    def create_sandbox_env_var(
        ...
        return self.server.sandbox_config_manager.create_sandbox_env_var(
            env_var_create=env_var_create, sandbox_config_id=sandbox_config_id, actor=self.server.user_manager.get_user_by_id(
                self.user.id)
        )
    ...
    def update_sandbox_env_var(
        ...
        return self.server.sandbox_config_manager.update_sandbox_env_var(
            env_var_id=env_var_id, env_var_update=env_var_update, actor=self.server.user_manager.get_user_by_id(
                self.user.id)
        )
    ...
    def add_agent_memory_block(self, agent_id: str, create_block: CreateBlock) -> Memory:
        ...
        agent = self.server.agent_manager.attach_block(
            agent_id=agent_id, block_id=block.id, actor=self.server.user_manager.get_user_by_id(self.user.id))
        return agent.memory
