...
class AgentInterface(ABC):
    """Interfaces handle Violet-related events (observer pattern)
    ...
    def user_message(self, msg: str, msg_obj: Optional[Message] = None):
        """Violet receives a user message"""
        raise NotImplementedError

    @abstractmethod
    def internal_monologue(self, msg: str, msg_obj: Optional[Message] = None):
        """Violet generates some internal monologue"""
        raise NotImplementedError

    @abstractmethod
    def assistant_message(self, msg: str, msg_obj: Optional[Message] = None):
        """Violet uses send_message"""
        raise NotImplementedError

    @abstractmethod
    def function_message(self, msg: str, msg_obj: Optional[Message] = None):
        """Violet calls a function"""
        raise NotImplementedError
        ...
        if msg.startswith("Success"):
            ...
            if debug:
                ...
                if match:
                        ...
                        try:
                            ...
                            if function_name == "archival_memory_search":
                                ...
                                if STRIP_UI:
                                    ...
                                    print(
                                        f"{Fore.RED}{output}{Style.RESET_ALL}")
                    ...
                    printd(
                        f"{CLI_WARNING_PREFIX}did not recognize function message")
                    printd_function_message("", msg)
        else:
            try:
...
class QueuingInterface(AgentInterface):
    ...
    def _queue_push(self, message_api: Union[str, dict], message_obj: Union[Message, None]):
        ...
        if isinstance(message_api, str):
            ...
            if message_api == "STOP":
                ...
                raise ValueError(
                    f"Unrecognized string pushed to buffer: {message_api}")
            ...
            raise ValueError(
                f"Unrecognized type pushed to buffer: {type(message_api)}")
    ...
    def error(self, error: str):
        """Enqueue a special stop message"""
        self._queue_push(
            message_api={"internal_error": error}, message_obj=None)
        self._queue_push(message_api="STOP", message_obj=None)
